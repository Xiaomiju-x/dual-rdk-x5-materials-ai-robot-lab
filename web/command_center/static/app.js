...
window._accessRole='public';
function hasReviewerAccess(){ return ['judge','member','admin'].includes(window._accessRole); }
const PUBLIC_VIEW_KEYS=new Set(['home','status','atlas','brain','models','assets','twin','more']);
function applyAudienceRole(role){
  const publicOnly=!['judge','member','admin'].includes(role);
  const minimalMode=document.documentElement.dataset.site32VisualMode==='minimal';
  const reduceSurface=publicOnly&&minimalMode;
  window._accessRole=publicOnly?'public':role;
  document.body.dataset.audience=publicOnly?'public':(role==='judge'?'judge':'internal');
  document.querySelectorAll('[data-k]').forEach(element=>{
    const key=element.dataset.k||'';
    element.hidden=reduceSurface&&!PUBLIC_VIEW_KEYS.has(key);
  });
  const menu=document.getElementById('moreMenu');
  if(menu) menu.querySelectorAll('button[onclick]').forEach(button=>{
    const match=(button.getAttribute('onclick')||'').match(/go\('([^']+)'\)/);
    if(match) button.hidden=reduceSurface&&!PUBLIC_VIEW_KEYS.has(match[1]);
    if((button.getAttribute('onclick')||'').includes('/logout')) button.hidden=publicOnly;
    if((button.getAttribute('onclick')||'').includes('palOpen')) button.hidden=reduceSurface;
  });
  const workOrder=document.querySelector('#homeSearchKind option[value="work_order"]');
  if(workOrder){ workOrder.hidden=publicOnly; workOrder.disabled=publicOnly; }
  const badge=document.getElementById('roleBadge');
  if(badge){
    badge.textContent=publicOnly?uiText('公开访客 · 只读','Public visitor · read-only'):role==='judge'?uiText('评委视角 · 只读','Judge view · read-only'):uiText('已认证工作区','Authenticated workspace');
...
const FRAME_FALLBACK = {
  ...
  car:{ icon:'🚗', title:'车载脑 · Lab-FSD v2', sub:'BEV 占据 · 时序世界模型 · 安全门禁', chips:['Nav2/MPPI 控制边界','BPU anomaly AE ≈1.7ms','策略 Token','断网本地 Agent'] },
  arm:{ icon:'🦾', title:'机械臂 · WorkCockpit', sub:'arm01 视觉冗余与投袋 · arm02 并发四周期研磨', chips:['视觉门控','安全姿态','Capsule 防撞','批次回填'] },
};

function toast(m, type){ const t=document.getElementById('toast'); t.textContent=m;
  t.className='toast show'+(type?' '+type:'');
  clearTimeout(t._h); t._h=setTimeout(()=>t.classList.remove('show'),2200); }
const MOTION_PREF=window.matchMedia?window.matchMedia('(prefers-reduced-motion: reduce)'):{matches:false};
const MOTION_FORCE_REDUCE=new URLSearchParams(location.search||'').get('motion')==='reduce';
if(MOTION_FORCE_REDUCE) document.documentElement.dataset.motionMode='reduce';
function motionReduced(){ return MOTION_FORCE_REDUCE||!!MOTION_PREF.matches; }
...
function tweenNum(el, target, fmt){
  ...
  el.dataset.v=target;
  if(motionReduced()){ el.innerHTML=fmt(target); return; }
...
const BOOT = {
  home:{ icon:'⬢', title:'智研指挥中心', sub:'三机异构协同 · 统一控制台', ms:330, foot:'指挥链路',
    stages:['接入三机异构链路','加载 AI 脑算力档案','同步具身真机闭环与 SLAM/shadow 摘要','校验双机械臂复赛协同记录','指挥中心就绪'] },
  ...
  story:{ icon:'📖', title:'项目故事 · 从痛点到飞轮', sub:'15 个月 → 15 分钟', ms:330, foot:'项目故事',
    stages:['梳理实验室痛点','装载三机异构分工','绘制闭环证据飞轮','汇总生态位定位','项目故事就绪'] },
  mq:{ icon:'📋', title:'闭环实验队列 · Mission Tracker', sub:'多阶段实验编排', ms:320, foot:'实验队列',
  ...
  replay:{ icon:'🎞', title:'闭环实验回放', sub:'10 阶段样品流 · 故障注入', ms:330, foot:'实验回放',
    stages:['读取 trace_id / 工单','装载 10 阶段实验时间线','同步车 / arm01 / 执行器回放态','挂接 5 类故障注入','生成样品流证据表','回放就绪'] },
  ...
  car:{  icon:'🚗', title:'车载脑 · NavCockpit', sub:'Lab-FSD v2 · BEV 世界模型 · 安全门禁', ms:340, foot:'驾驶舱',
    stages:['初始化 WebGL 三维场景','读取遥测/回放来源','加载 Lab-FSD v2 BEV','校验安全门禁','读取 Nav2/MPPI 控制边界','驾驶舱就绪'] },
  arm:{  icon:'🦾', title:'机械臂 · WorkCockpit', sub:'arm01 视觉冗余与投袋 · arm02 并发四周期研磨', ms:340, foot:'机械臂工位',
    stages:['初始化 arm01/arm02 工位场景','连接工位 socket.io','加载 DH 正运动学','校验防撞 capsule 互锁','工位就绪'] },
};
let bootTimers=[], booting=false, bootAfter=null, bootToken=0;
function cancelBoot(hide){
  bootToken++;
  bootTimers.forEach(clearTimeout); bootTimers=[];
  booting=false; bootAfter=null;
  if(hide!==false){
    const ov=document.getElementById('boot');
    if(ov) ov.classList.remove('show','out');
  }
}
function runBoot(k, after, generation){
  if(motionReduced()){ if(after && navigationCurrent(generation)) after(); return; }
  cancelBoot(false);
  ...
  ov.classList.remove('out'); ov.classList.add('show'); booting=true;
  bootAfter={token:token,generation:generation,callback:after};
  function setRow(i){
  ...
  setRow(0);
  const step=Math.min(b.ms||340,180);
  for(let i=1;i<=b.stages.length;i++){
    bootTimers.push(setTimeout(()=>{
      if(token!==bootToken) return;
      if(i<b.stages.length) setRow(i); else finishBoot(token);
    }, step*i));
  }
}
function finishBoot(token){
  token=token||bootToken;
  if(!booting || token!==bootToken) return; booting=false;
  bootTimers.forEach(clearTimeout); bootTimers=[];
  const ov=document.getElementById('boot');
  ...
  bootTimers.push(setTimeout(()=>{
    if(token!==bootToken) return;
    ov.classList.remove('show','out');
    const pending=bootAfter; bootAfter=null; bootTimers=[];
    if(pending && pending.token===token && navigationCurrent(pending.generation) && pending.callback){
      pending.callback();
    }
  }, motionReduced()?0:220));
}
...
let _routeBusy=false, _routeQueued=null, _activeViewTransition=null, _routeActiveGeneration=0;
let _navigationGeneration=0;
function navigationCurrent(generation){ return generation===_navigationGeneration; }
function routeTransitionDone(generation){
  if(generation!==_routeActiveGeneration) return;
  clearRouteTransitionState();
  site24MotionState('idle');
  _routeBusy=false;
  _activeViewTransition=null;
  const q=_routeQueued; _routeQueued=null;
  delete document.body.dataset.routeQueued;
  if(q && navigationCurrent(q.opts.generation) && (q.k!==cur || q.opts.force)){
    setTimeout(()=>transitionGo(q.k, q.opts||{}), 0);
  }else if(q && navigationCurrent(q.opts.generation) && q.opts && q.opts.after){
    q.opts.after();
  }
}
function transitionGo(k, opts){
  opts=opts||{};
  const generation=opts.generation||_navigationGeneration;
  opts.generation=generation;
  if(!navigationCurrent(generation)) return;
  if(_routeBusy){
    _routeQueued={k:k, opts:opts};
    document.body.dataset.routeQueued=k;
    if(_activeViewTransition && _activeViewTransition.skipTransition){
      try{ _activeViewTransition.skipTransition(); }catch(e){}
    }
    return;
  }
  _routeBusy=true;
  _routeActiveGeneration=generation;
  ...
  if(document.startViewTransition && !reduced){
    try{
      ...
      vt.finished.finally(()=>routeTransitionDone(generation));
      return;
    }catch(e){}
  }
  document.body.classList.add('route-fallback');
  apply();
  const active=PAGES[k] ? document.getElementById(PAGES[k]) : null;
  if(active){
    active.classList.remove('route-entering');
    void active.offsetWidth;
    active.dataset.routeEnterGeneration=String(generation);
    active.classList.add('route-entering');
    setTimeout(()=>{
      if(active.dataset.routeEnterGeneration!==String(generation)) return;
      active.classList.remove('route-entering');
      delete active.dataset.routeEnterGeneration;
    }, 220);
  }
  routeTransitionDone(generation);
}
function site24MotionState(state){
  ...
  if(state==='routing'){
    ...
    site24MotionState._t=setTimeout(()=>site24MotionState('idle'), 360);
  }
}
document.addEventListener('visibilitychange', ()=>{
  ...
  if(!document.hidden){
    pollFleet();
    if(cur==='home'||cur==='status') loadPublicStatus();
    if(cur==='home') pollKpi();
  }
});
function go(k, opts){
  ...
  opts.generation=generation;
  if(!navigationCurrent(generation)) return;
  document.body.dataset.routeIntent=k;
  document.body.dataset.routeIntentGeneration=String(generation);
  if(booting) cancelBoot(true);
  if(k===cur && !opts.force){
    if(opts.path) ensureFrame(k, opts.path);
    if(opts.after && navigationCurrent(generation)) opts.after();
    return;
  }
  const from=cur;
  if(!PAGES[k]){
    ensureFrame(k, opts.path);   // 系统视图: 动画背后预加载真机/镜像 (可深链子路由)
    runBoot(k, ()=>transitionGo(k,{from:from,after:opts.after,motion:'system-boot',force:opts.force,generation:generation}),generation);
    return;
  }
  if(opts.boot){
    runBoot(k, ()=>transitionGo(k,{from:from,after:opts.after,motion:'system-boot',force:opts.force,generation:generation}),generation);
  ...
  transitionGo(k,{from:from,after:opts.after,motion:opts.motion,force:opts.force,generation:generation});
}

function installRapidNavCommit(){
  const nav=document.getElementById('iaStrip');
  if(!nav || nav.dataset.rapidNavCommit==='true') return;
  nav.dataset.rapidNavCommit='true';
  let armed=null, committed=null;
  nav.addEventListener('mousedown', event=>{
    const chip=event.target&&event.target.closest ? event.target.closest('.ia-chip[data-k]') : null;
    if(!chip || event.button!==0){
      armed=null; return;
    }
    armed=chip;
  }, true);
  nav.addEventListener('mouseup', event=>{
    const chip=event.target&&event.target.closest ? event.target.closest('.ia-chip[data-k]') : null;
    if(!armed || !chip || armed!==chip){ armed=null; return; }
    armed=null;
    committed={chip:chip, route:chip.dataset.k, at:performance.now()};
    go(committed.route);
  }, true);
  nav.addEventListener('click', event=>{
    const chip=event.target&&event.target.closest ? event.target.closest('.ia-chip[data-k]') : null;
    if(!committed || chip!==committed.chip || performance.now()-committed.at>1000) return;
    committed=null;
...
function revealView(k){
  ...
  document.body.dataset.routeSettled=k;
  if(window.livingSetView) window.livingSetView(k);
  document.querySelectorAll('#nav .pill,.ia-chip,.mtab').forEach(p=>{
    const active=p.dataset.k===k;
  ...
  if(k==='home'){
    scheduleThreeSceneLoad();
    if(window.threeSceneKick) window.threeSceneKick();
    loadPublicStatus();
    pollKpi();
  }
  ...
  syncStreamForRoute();
  announceRouteChange(k);
}

function announceRouteChange(k){
  const active=PAGES[k] ? document.getElementById(PAGES[k]) : frameEls[k];
  const heading=active&&active.querySelector ? active.querySelector('h1,h2,.hero h1,.hero h2') : null;
  ...
  const product=uiText('XRD 智慧实验室','XRD Smart Lab');
  document.title=(label==='XRD 智慧实验室'||label==='XRD Smart Lab')
    ? uiText('XRD 智慧实验室 · 材料预测与实验闭环证据库','XRD Smart Lab · Materials Prediction and Closed-Loop Evidence')
    : label+' · '+product;
  const live=document.getElementById('routeAnnouncer');
  if(live){
    live.textContent='';
    window.requestAnimationFrame(()=>{ live.textContent=uiText('已进入 ','Opened ')+label; });
  }
}
...
const KIOSK={ on:false, idx:0, timer:null, paused:false, dwell:13000,
  pl:[ {k:'home',t:'三机异构总览'}, {k:'highlight',t:'技术亮点 · 5 大实测'},
       ...
       {k:'car',t:'车载脑 · SLAM / 影子规划证据'}, {k:'arm',t:'机械臂 · arm01 工位'},
       {k:'mq',t:'批次工单闭环'} ] };
function kioskStart(){
...
async function notifyToggle(){
  if(!hasReviewerAccess()){ toast(uiText('通知中心仅在评审或内部只读层开放','Notifications require reviewer access'),'warn'); return; }
...
async function notifyCheck(){
  if(!hasReviewerAccess() || document.hidden || !NOTIFY.on || !('Notification' in window) || Notification.permission!=='granted') return;
  try{
...
function ncToggle(){
  if(!hasReviewerAccess()){ toast(uiText('通知中心仅在评审或内部只读层开放','Notification center requires reviewer access'),'warn'); return; }
  NC_OPEN=!NC_OPEN;
  document.getElementById('ncMask').classList.toggle('show', NC_OPEN);
  if(NC_OPEN) ncLoad();
}
function ncTab(t){ NC_TAB=t; NC_DIAG=false;
  document.querySelectorAll('.nc-tab').forEach(x=>{
    const active=x.dataset.t===t; x.classList.toggle('on',active); x.setAttribute('aria-selected',active?'true':'false');
  });
  ncLoad();
}
let NC_DIAG=false;
...
async function pollFleet(){
  ...
  try{
    ...
    ['lab','car','arm'].forEach(k=>{
      ...
      const serving=String(s.serving||d.snapshot?.state||'unknown').toLowerCase();
      ...
      if(tx){
        const labels={mirror:'镜像快照',replay:'回放快照',mock:'模拟快照',stale:'陈旧快照',offline:'离线',down:'离线',unknown:'未知 (当前无快照)',planned:'规划中'};
        tx.textContent=on?'真机在线':(labels[serving]||'未知 (当前无快照)');
        tx.style.color=on?'#059669':serving==='mirror'||serving==='replay'||serving==='stale'?'#b45309':'#8593ad';
      }
...
setInterval(pollFleet, 30000); pollFleet();

/* ---- Site9 R2: public status ---- */
function psClass(status){
  const s=String(status||'unknown').toLowerCase();
  if(s==='operational') return 'ok';
  if(s==='mirror'||s==='replay'||s==='planned'||s==='degraded') return 'warn';
...
function uiTerm(v){
  ...
  const m={
    ...
    'dual-arm sweep zone is represented as a protected boundary':'机械臂工位扫掠区以受保护边界呈现',
    ...
    'dual-arm sweep zone or interlock state is not clear':'机械臂工位扫掠区或互锁状态未清空',
    ...
    'f407 lift-stage replay evidence':'F407 升降台回放证据',
    'f407 actuator replay evidence':'F407 执行器回放证据','f407 servo-state replay evidence':'F407 舵机状态回放证据',
    'f407 magnet-state replay evidence':'F407 电磁铁状态回放证据',
    ...
    'ai brain':'AI 脑','vehicle brain':'车载脑','dual-arm station':'机械臂工位','characterization':'表征',
    'browser + ai brain':'浏览器 + AI 脑','ai brain x5':'AI 脑 X5','command center':'指挥中心',
    'vehicle + dual arms':'车载脑 + arm01 工位','researcher':'研究员',
    ...
    'lab-fsd replay, nav2/mppi authority, sample route':'Lab-FSD 回放、Nav2/MPPI 控制边界、样品路线',
    ...
    'f407 lift-stage replay / documented readiness':'F407 升降台回放 / 文档化就绪',
    'f407 magnet-state replay':'F407 吸附状态回放','relay extend/retract replay':'继电器伸出/缩回回放',
    ...
    'digital twin / experiment replay lifecycle':'数字孪生 / 实验回放生命周期','dual-arm station and sample flow evidence':'arm01 工位与样品流证据',
...
function psStatusLabel(status){
  ...
  return {operational:'正常',degraded:'降级',mirror:'镜像',replay:'回放',planned:'计划',offline:'离线',unknown:'未知'}[s] || status || '未知';
}
function psTime(ts){
  if(!ts) return '—';
...
function sceneTelemetrySourceLabel(c){
  ...
  return {
    live:uiText('真机','live'), stale:uiText('陈旧','stale'), mirror:uiText('镜像','mirror'),
    replay:uiText('回放','replay'), planned:uiText('计划','planned'),
    offline:uiText('离线','offline'), unknown:uiText('未知','unknown')
  }[src] || src;
}
...
function homeNodeList(d){
  ...
  return [
    ...
    {key:'car', label:uiText('车载脑','Vehicle brain'), focus:'car', page:'fsd', sub:'SLAM / Lab-FSD shadow / Nav2', comp:pick('car',['car brain'])},
    {key:'arm', label:uiText('双臂工位','dual-arm station'), focus:'arm', page:'replay', sub:'arm01 bag drop / arm02 four-cycle grinding', comp:pick('arm',['arm workstation','arm01','arm02'])},
...
function homeOsUpdate(d){
  const s=(d&&d.summary)||{}, cls=psClass(s.status), rel=s.release||'site32-global-commercial-v1.13-20260720';
  ...
  const matrixNodes=[nodes[0],nodes[1],nodes[2],nodes[6]].filter(Boolean);
  const matrix=document.getElementById('homeStatusMatrix');
  if(matrix) matrix.innerHTML=matrixNodes.map(n=>{
  ...
  const swName='cmdcenter-shell-v96-site32-global-commercial-v1.13';
...
async function loadSite29Portal(){
  const metrics=document.getElementById('site29Metrics');
  const patterns=document.getElementById('site29Patterns');
  const objects=document.getElementById('site29Objects');
  const trust=document.getElementById('site29Trust');
  const commitments=document.getElementById('site30Commitments');
  const negative=document.getElementById('site30NegativeControls');
  const ladder=document.getElementById('site30EvidenceLadder');
  if(!metrics && !patterns && !objects && !trust && !commitments && !negative && !ladder) return;
  let d=null;
  try{ d=await fetch('/api/research_portal',{cache:'no-store'}).then(r=>r.json()); }catch(e){ d=null; }
  if(!d) return;
  if(metrics){
    metrics.innerHTML=(d.hero_metrics||[]).map(m=>
      '<article><b>'+uiEsc(m.value)+'</b><span>'+uiEsc(m.label||'')+'</span><i>'+uiEsc(m.source||'')+'</i></article>'
    ).join('');
  }
  if(patterns){
    patterns.innerHTML=(d.system_cards||d.top_platform_patterns||[]).slice(0,6).map(p=>
      '<article><b>'+uiEsc(p.site||'')+'</b><p>'+uiEsc(p.pattern||'')+'</p><i>'+uiEsc(p.reuse||'')+'</i></article>'
    ).join('');
  }
  if(objects){
    objects.innerHTML=(d.evidence_objects||[]).slice(0,7).map(o=>
      '<article class="sp-dossier" role="button" tabindex="0" data-evidence-id="'+esc(o.evidence_id||'')+'" aria-label="'+uiEsc(uiText('打开证据对象 ','Open evidence object ')+(o.title||o.evidence_id||''))+'"><span>'+uiEsc(((o.origin||[])[0])||o.legacy_evidence_label||'evidence')+'</span><b>'+uiEsc(o.title||o.kind||'evidence')+'</b><p>'+uiEsc(o.claim||'')+'</p>'+
      '<dl><div><dt>ID</dt><dd>'+uiEsc(o.evidence_id||'')+'</dd></div><div><dt>验证</dt><dd>'+uiEsc(o.validation_status||((o.validation||{}).result)||'unverified')+'</dd></div>'+
      '<div><dt>来源</dt><dd>'+uiEsc(((o.origin||[]).join(' / '))||o.source_label||'unknown')+'</dd></div><div><dt>限制</dt><dd>'+uiEsc((o.limitations||[]).join('；')||o.limitations_short||'public-safe summary only')+'</dd></div></dl>'+
      '<i>'+uiEsc(o.citation_text||'')+'</i></article>'
    ).join('');
    objects.querySelectorAll('[data-evidence-id]').forEach(el=>{
      const open=()=>detailOpen('evidence',el.dataset.evidenceId||'');
  ...
  if(trust){
    const controls=((d.trust_center||{}).controls||[]);
    trust.innerHTML=controls.map(c=>
      '<article class="'+uiEsc(c.state||'')+'"><span>'+uiEsc(c.state||'')+'</span><b>'+uiEsc(c.id||'')+'</b><p>'+uiEsc(c.verification_method||'')+'</p><i>'+uiEsc(c.residual_risk||'')+'</i></article>'
    ).join('');
  }
  if(commitments){
    commitments.innerHTML=(d.open_science_commitments||[]).map(c=>
      '<article><b>'+uiEsc(c.name||'')+'</b><p>'+uiEsc(c.detail||'')+'</p><i>'+uiEsc(c.evidence||'')+'</i></article>'
    ).join('');
  }
  if(negative){
    negative.innerHTML=(d.negative_controls||[]).map(c=>
      '<article class="'+uiEsc(c.state||'')+'"><span>'+uiEsc(c.state||'')+'</span><b>'+uiEsc(c.id||'')+'</b><p>'+uiEsc(c.check||'')+'</p></article>'
    ).join('');
  }
  if(ladder){
    ladder.innerHTML=(d.evidence_modes||d.evidence_ladder||[]).map(x=>
      '<article><span>'+uiEsc(x.level||'')+'</span><b>'+uiEsc(x.name||'')+'</b><p>'+uiEsc(x.use||'')+'</p></article>'
    ).join('');
  }
}

/* ---- Public Research Commons: curated entry points over public-safe DTOs. ---- */
let _researchCollections=[];
let _researchCommonsFilter='all';
let _researchCommonsOpen='';
function researchCommonsLabel(item,key){
  if(!item) return '';
  return uiLang()==='en' ? (item[key+'_en']||item[key]||'') : (item[key]||item[key+'_en']||'');
}
function researchCommonsHref(value,fallback){
  const href=String(value||'');
  return href.startsWith('/')&&!href.startsWith('//')&&!href.includes('://') ? href : (fallback||'#');
}
function researchCommonsCountText(item){
  const counts=item.counts_by_kind||{};
  const names={material:uiText('材料','materials'),evidence:uiText('证据','evidence'),page:uiText('入口','pages')};
  const bits=Object.keys(counts).sort().map(k=>counts[k]+' '+(names[k]||k));
  return bits.join(' · ')||uiText('暂无公开对象','No public objects');
}
function researchCommonsFiltered(){
  if(_researchCommonsFilter==='all') return _researchCollections;
  return _researchCollections.filter(item=>item.scope===_researchCommonsFilter);
}
function researchCommonsRender(){
  const host=document.getElementById('researchCommons');
  const list=document.getElementById('researchCommonsList');
  const summary=document.getElementById('researchCommonsSummary');
  if(!host||!list) return;
  const items=researchCommonsFiltered();
  if(_researchCommonsOpen&&!items.some(item=>item.collection_id===_researchCommonsOpen)) researchCommonsClose(true);
  host.setAttribute('aria-busy','false');
  if(summary) summary.textContent=uiText('显示 ','Showing ')+items.length+uiText(' 个公开专题',' public collections');
  if(!items.length){
    list.innerHTML='<div class="prc-empty"><b>'+uiText('当前筛选没有公开专题','No public collection matches this filter')+'</b><span>'+uiText('选择“全部专题”查看完整公开集合。','Choose “All collections” to restore the public index.')+'</span></div>';
    researchCommonsClose(false);
    return;
  }
  list.innerHTML=items.map(item=>{
    const title=researchCommonsLabel(item,'title');
    const description=researchCommonsLabel(item,'description');
    return '<button class="prc-object-row" type="button" data-prc-id="'+esc(item.collection_id||'')+'" data-scope="'+esc(item.scope||'public_site')+'" aria-expanded="'+(item.collection_id===_researchCommonsOpen?'true':'false')+'">'+
      '<span class="prc-swatch" aria-hidden="true"></span><span class="prc-object-title"><b>'+uiEsc(title)+'</b><span>'+uiEsc((item.topics||[]).join(' · '))+'</span></span>'+
      '<span class="prc-object-copy">'+uiEsc(description)+'</span><span class="prc-provenance"><b>'+uiEsc(item.visibility||'public-read-only')+'</b> · '+uiEsc(researchCommonsCountText(item))+'</span><span class="prc-open" aria-hidden="true">›</span></button>';
  }).join('');
  list.querySelectorAll('[data-prc-id]').forEach(button=>button.addEventListener('click',()=>researchCollectionOpen(button.dataset.prcId||'')));
  if(_researchCommonsOpen) researchCollectionOpen(_researchCommonsOpen,false);
}
function researchCommonsFilter(scope,trigger){
  _researchCommonsFilter=String(scope||'all');
  document.querySelectorAll('[data-prc-filter]').forEach(button=>{
    const active=button.dataset.prcFilter===_researchCommonsFilter;
    button.classList.toggle('active',active);
    button.setAttribute('aria-pressed',active?'true':'false');
  });
  researchCommonsRender();
  if(trigger&&trigger.focus) trigger.focus({preventScroll:true});
}
function researchCommonsClose(updateUrl){
  _researchCommonsOpen='';
  const detail=document.getElementById('researchCommonsDetail');
  if(detail){ detail.hidden=true; detail.innerHTML=''; }
  document.querySelectorAll('[data-prc-id]').forEach(button=>button.setAttribute('aria-expanded','false'));
  if(updateUrl!==false){
    const url=new URL(location.href); url.searchParams.delete('collection');
    history.replaceState(history.state,'',url.pathname+url.search+url.hash);
  }
}
function researchCollectionOpen(collectionId,updateUrl){
  const item=_researchCollections.find(row=>row.collection_id===collectionId);
  const detail=document.getElementById('researchCommonsDetail');
  if(!item||!detail){ if(collectionId) researchCommonsClose(updateUrl); return; }
  _researchCommonsOpen=collectionId;
  document.querySelectorAll('[data-prc-id]').forEach(button=>button.setAttribute('aria-expanded',button.dataset.prcId===collectionId?'true':'false'));
  const members=(item.members||[]).slice(0,6);
  const limit=(uiLang()==='en'?(item.limitations_en||item.limitations):(item.limitations||item.limitations_en)||[])[0]||'';
  detail.innerHTML='<div class="prc-detail-head"><div><h3>'+uiEsc(researchCommonsLabel(item,'title'))+'</h3><p>'+uiEsc(item.collection_id||'')+' · '+uiEsc(researchCommonsCountText(item))+'</p></div><button type="button" data-prc-close aria-label="'+uiText('关闭专题详情','Close collection details')+'">×</button></div>'+
    '<div class="prc-member-list">'+members.map(member=>'<a href="'+uiEsc(researchCommonsHref(member.canonical_url,'#'))+'"><span>'+uiEsc(member.kind||'object')+'</span><b>'+uiEsc(researchCommonsLabel(member,'title')||member.object_id||'object')+'</b></a>').join('')+'</div>'+
    '<p class="prc-limit"><b>'+uiText('适用边界：','Boundary: ')+'</b>'+uiEsc(limit)+' <a href="'+uiEsc(researchCommonsHref(item.canonical_url,'/api/research_collections'))+'" target="_blank" rel="noopener">API</a></p>';
  detail.hidden=false;
  const close=detail.querySelector('[data-prc-close]'); if(close) close.addEventListener('click',()=>researchCommonsClose(true));
  if(updateUrl!==false){
    const url=new URL(location.href); url.searchParams.set('collection',collectionId);
    history.replaceState(history.state,'',url.pathname+url.search+url.hash);
  }
}
async function loadResearchCommons(){
  ...
  try{
    const response=await fetch('/api/research_collections?limit=20',{cache:'no-store'});
    if(!response.ok) throw new Error('HTTP '+response.status);
    const data=await response.json();
    _researchCollections=Array.isArray(data.items)?data.items:[];
    const requested=new URLSearchParams(location.search).get('collection')||'';
    _researchCommonsOpen=_researchCollections.some(item=>item.collection_id===requested)?requested:'';
    researchCommonsRender();
  }catch(error){
    host.setAttribute('aria-busy','false');
    if(summary) summary.textContent=uiText('专题接口暂不可用','Collections API unavailable');
    list.innerHTML='<div class="prc-empty error"><b>'+uiText('公开专题暂时无法加载','Public collections could not be loaded')+'</b><button type="button" data-prc-retry>'+uiText('重新加载','Retry')+'</button></div>';
    const retry=list.querySelector('[data-prc-retry]'); if(retry) retry.addEventListener('click',loadResearchCommons);
  }
}
function psRender(d){
  if(!d) return;
  ...
  if(ab) ab.innerHTML=['lab','car','arm'].map(k=>{
    const label={lab:uiText('AI 脑','AI brain'),car:uiText('车载脑','Car brain'),arm:uiText('机械臂工位','Arm workstation')}[k]||k, row=av[k]||{};
...
setInterval(()=>{ if(!document.hidden&&(cur==='home'||cur==='status')) loadPublicStatus(); }, 30000);
loadPublicStatus();
...
const KPI_FALLBACK={ source:'unknown', kpi:{
  ci_coverage_pct:null, ci_narrowing_pct:null, audit_valid:null, audit_total:null,
  audit_intact:null, predictions:null
}};
function applyKpi(d){
  d=d||{};
  ...
  const tw=(id,v,dec,suf)=>{ const e=document.getElementById(id);
    if(!e) return;
    if(v!=null) tweenNum(e, +v, x=>(dec?x.toFixed(dec):Math.round(x))+(suf||''));
    else e.textContent='—'; };
  ...
  if(src){ const real=source==='real'||source==='live', mirrored=['mirror','replay','mock','stale'].includes(source);
    src.className='kpi-src '+(real?'real':mirrored?'mirror':'unknown');
    src.textContent = real?'● 真机快照':mirrored?'◐ '+uiTerm(source)+'快照':'○ 当前无可信快照';
    src.title = real?'数据来自已标注时间的在线设备快照':mirrored?'显示带来源标签的镜像、回放或陈旧快照':'采样器尚未提供可验证数据'; }
}
...
setInterval(()=>{ if(cur==='home') pollKpi(); }, 30000); pollKpi();
...
function streamWanted(){ return !document.hidden&&cur==='ops'; }
function stopStream(){
  if(_esRetry){ clearTimeout(_esRetry); _esRetry=0; }
  if(_es){ try{_es.close();}catch(e){} _es=null; }
}
function syncStreamForRoute(){ if(streamWanted()) startStream(); else stopStream(); }
function startStream(){
  if(!window.EventSource||!streamWanted()||_es) return;
  try{
    ...
    _es.onerror=()=>{
      if(_es){ try{_es.close();}catch(e){} _es=null; }
      if(streamWanted()&&!_esRetry) _esRetry=setTimeout(()=>{ _esRetry=0; startStream(); },10000);
    };
  }catch(e){}
}
syncStreamForRoute();
...
/* ---- H23 离线降级指示 ---- */
let offProbeSeq=0;
function offSetState(off){
  const bar=document.getElementById('offbar'); if(!bar) return false;
...
async function offApply({notify=false}={}){
  const seq=++offProbeSeq;
  let off=false;
  try{
    // HEAD is intentionally outside the Service Worker GET cache path. Any HTTP
    // response proves that the browser can still reach the same-origin edge.
    await fetch('/api/public_status?online_probe='+Date.now(),{
      method:'HEAD', cache:'no-store', credentials:'same-origin'
    });
  }catch(e){ off=true; }
  if(seq!==offProbeSeq) return !off;
  const recovered=offSetState(off);
  if(recovered && notify) toast('✓ 网络已恢复','ok');
  return !off;
}
window.addEventListener('online', ()=>{ offApply({notify:true}).then(ok=>{
  if(ok){ try{ pollFleet();pollKpi(); }catch(e){} }
}); });
window.addEventListener('offline', ()=>{ setTimeout(()=>offApply(), 300); });
offApply();
...
function moreToggle(e){ if(e) e.stopPropagation();
  ...
  if(trigger){ trigger.classList.toggle('on', open); trigger.setAttribute('aria-expanded',open?'true':'false'); }
...
function moreClose(){ const m=document.getElementById('moreMenu'); if(m) m.classList.remove('show');
  ...
  const b=document.getElementById('btnMore'); if(b){ b.classList.remove('on'); b.setAttribute('aria-expanded','false'); } document.body.classList.remove('more-open'); }
function moreGo(fn){ moreClose(); try{ fn(); }catch(e){} }
document.addEventListener('click', e=>{ if(!e.target.closest('.more-wrap')) moreClose(); });

function site32SyncModeButton(){
  const mode=document.documentElement.dataset.site32PresentationMode||
    (document.documentElement.dataset.site32VisualMode==='minimal'?'minimal':'vivid');
  const button=document.getElementById('site32ModeToggle');
  const label=document.getElementById('site32ModeLabel');
  const names={vivid:uiText('完整','Full'),defense:uiText('答辩','Defense'),minimal:uiText('极简','Minimal')};
  const icons={vivid:'◐',defense:'▣',minimal:'≡'};
  const next={vivid:'defense',defense:'minimal',minimal:'vivid'};
  const descriptions={
    vivid:uiText('当前为完整液态玻璃模式，点击切换到答辩静态玻璃','Full liquid glass active; switch to defense static glass'),
    defense:uiText('当前为答辩模式，背景静态且功能完整；点击切换到极简模式','Defense mode active with static background and full functions; switch to minimal'),
    minimal:uiText('当前为极简降级模式，点击恢复完整液态玻璃','Minimal fallback active; restore full liquid glass')
  };
  if(label) label.textContent=names[mode]||names.vivid;
  if(button){
    button.dataset.mode=mode;
    button.title=descriptions[mode]||descriptions.vivid;
    button.setAttribute('aria-label',button.title);
    const icon=button.querySelector('.mode-toggle-icon');
    if(icon) icon.textContent=icons[mode]||icons.vivid;
  }
}
function site32QuickModeToggle(){
  const mode=document.documentElement.dataset.site32PresentationMode||
    (document.documentElement.dataset.site32VisualMode==='minimal'?'minimal':'vivid');
  const next={vivid:'defense',defense:'minimal',minimal:'vivid'}[mode]||'vivid';
  if(window.Site32&&window.Site32.appearance) window.Site32.appearance.setMode(next);
  else {
    const visual=next==='minimal'?'minimal':'vivid';
    document.documentElement.dataset.site32PresentationMode=next;
    document.documentElement.dataset.site32VisualMode=visual;
    document.body.dataset.site32PresentationMode=next;
    document.body.dataset.site32VisualMode=visual;
    try{ localStorage.setItem('cmdcenter.visualMode.v3',next); }catch(e){}
    window.dispatchEvent(new CustomEvent('site32:appearance-change',{detail:{requested:next,effective:next,source:'fallback'}}));
  }
  applyAudienceRole(window._accessRole||'public');
  site32SyncModeButton();
}
window.addEventListener('site32:appearance-change',()=>{
  applyAudienceRole(window._accessRole||'public');
  site32SyncModeButton();
});
// 后台轻量轮询: 给顶栏 ✅ 按钮一个红/黄角标 (NO-GO/注意), 不打扰当前页
async function pfPipPoll(){
  if(!hasReviewerAccess() || document.hidden) return;
  try{ const d=await fetch('/api/preflight',{cache:'no-store'}).then(r=>r.json()); if(d.summary) pfPipApply(d.summary); }catch(e){}
}
let _roleRuntimeStarted=false, _pfPipTimer=0;
function startRoleBoundRuntime(){
  if(_roleRuntimeStarted || !hasReviewerAccess()) return;
  _roleRuntimeStarted=true;
  loadAlarms();
  if(NOTIFY.on){ NOTIFY._init=false; notifyCheck(); }
  pfPipPoll();
  _pfPipTimer=setInterval(pfPipPoll,90000);
}
...
/* ---- G6 Closed-loop Experiment Replay ---- */
...
function benchmarkRender(){
  ...
  if(ts) ts.textContent=(d.release||'')+' · '+uiText('内部证据门禁','internal evidence gate')+' '+esc(d.overall_score??'—')+'/'+esc(d.score_max??100)+' · '+new Date().toLocaleTimeString('zh-CN');
  host.innerHTML=
    '<div class="benchmark-kpis">'+
      '<div><b>'+esc(d.overall_score??'—')+'/'+esc(d.score_max??100)+'</b><span>'+uiText('内部发布就绪度','internal release readiness')+'</span></div>'+
      '<div><b>'+esc(d.dimension_count??((d.dimensions||[]).length))+'</b><span>'+uiText('对标维度','benchmark dimensions')+'</span></div>'+
      '<div><b>'+esc((d.gates||[]).filter(g=>g.state==='verified').length)+'/'+esc((d.gates||[]).length)+'</b><span>'+uiText('已验证门禁','verified gates')+'</span></div>'+
      '<div><b>'+uiEsc(d.gate_state||'unknown')+'</b><span>'+uiText('最终门禁','final gate')+'</span></div>'+
    '</div>'+
    '<div class="benchmark-grid"><section><div class="benchmark-head"><h2>'+uiText('全球平台对标证据矩阵','Global Platform Evidence Matrix')+'</h2>'+fsdSource('internal rubric')+'</div>'+
      '<div class="benchmark-dims">'+((d.dimensions||[]).map(benchmarkDimHtml).join(''))+'</div></section>'+
      '<aside><div class="benchmark-head"><h2>'+uiText('验证门禁','Verification Gates')+'</h2>'+fsdSource('verified/manual')+'</div><div class="benchmark-gates">'+((d.gates||[]).map(benchmarkGateHtml).join(''))+'</div>'+
    ...
    '<div class="benchmark-bottom"><section><div class="benchmark-head"><h2>'+uiText('剩余诚实差距','Remaining Honest Gaps')+'</h2>'+fsdSource('not hidden')+'</div><div class="benchmark-bets">'+((d.next_bets||[]).map(x=>'<p>'+uiEsc(x)+'</p>').join(''))+'</div><p class="benchmark-score-note">'+uiEsc(d.score_type||'')+'</p></section>'+
...
function benchmarkDimHtml(x){
  ...
  return '<article class="'+fsdClass(x.state||'pass')+'">'+
    '<div class="benchmark-card-head"><span>'+uiEsc(x.label||x.key||'')+'</span><b>'+esc(x.score??'—')+'/100</b></div>'+
...
let _rbVoeExplain=null;
async function loadBrainExplain(){
  const host=document.getElementById('brainExplain'), voeHost=document.getElementById('rbVoeExplain');
  if(host) host.innerHTML='<div class="skel-rows"><div class="skel"></div><div class="skel"></div><div class="skel"></div></div>';
  if(voeHost) voeHost.innerHTML='<div class="skel-rows"><div class="skel"></div><div class="skel"></div><div class="skel"></div></div>';
  try{
    const results=await Promise.allSettled([
      fetch('/api/ai_brain/explain',{cache:'no-store'}).then(r=>{if(!r.ok) throw new Error('brain '+r.status);return r.json();}),
      fetch('/api/rb_voe/explain',{cache:'no-store'}).then(r=>{if(!r.ok) throw new Error('rb-voe '+r.status);return r.json();})
    ]);
    _brainExplain=results[0].status==='fulfilled'?results[0].value:{error:String(results[0].reason),pipeline:[],signals:[],boundaries:[],summary:{}};
    _rbVoeExplain=results[1].status==='fulfilled'?results[1].value:{error:String(results[1].reason)};
  }catch(e){
    _brainExplain={error:String(e), pipeline:[], signals:[], boundaries:[], summary:{}};
    _rbVoeExplain={error:String(e)};
  }
  brainRender();
  rbVoeRender();
}
...
function rbVoeShortHash(v){ const s=String(v||''); return s.length>18?s.slice(0,10)+'…'+s.slice(-8):s||'-'; }
function rbVoeRender(){
  const d=_rbVoeExplain||{}, host=document.getElementById('rbVoeExplain');
  if(!host) return;
  if(d.error || !d.policy){
    host.innerHTML='<div class="voe-unavailable"><b>'+uiText('X5-RB-VoE 证据不可用','X5-RB-VoE evidence unavailable')+'</b><span>'+uiText('公开快照未通过发布绑定校验；页面不会回填为实时状态。','The public snapshot did not pass release validation and is not replaced with live-looking data.')+'</span></div>';
    return;
  }
  const p=d.policy||{}, a=d.authority||{}, src=d.source||{}, dag=d.evidence_dag||{}, cmp=d.comparisons||{}, hold=d.hold_witness||{};
  const nodes=(dag.nodes||[]).map(n=>'<div class="voe-node"><span>'+uiEsc(n.kind||n.evidence_id||'evidence')+'</span><b>'+uiEsc(n.evidence_id||'')+'</b><i>'+uiEsc((n.failure_domains||[]).join(' · '))+'</i></div>').join('');
  const branches=(p.branches||[]).map((b,i)=>'<div class="voe-branch"><span>'+String(i+1).padStart(2,'0')+'</span><div><b>'+uiEsc(b.observation||'')+'</b><i>→ '+uiEsc(b.option_id||'')+'</i><small>'+uiEsc((b.scenario_ids||[]).join(' · '))+'</small></div></div>').join('');
  const comparisonRows=[
    [uiText('H2 自适应','H2 adaptive'),cmp.h2_adaptive&&cmp.h2_adaptive.risk,cmp.h2_adaptive&&cmp.h2_adaptive.terminal_closure?uiText('闭合','closed'):uiText('未闭合','open')],
    ['H1',cmp.h1&&cmp.h1.risk,cmp.h1&&cmp.h1.terminal_closure?uiText('闭合','closed'):uiText('未闭合','open')],
    [uiText('固定二步','fixed two-step'),(cmp.fixed_two_step&&cmp.fixed_two_step.complete)+'/'+(cmp.fixed_two_step&&cmp.fixed_two_step.enumerated),uiText('完整序列','complete sequences')],
    [uiText('全证据参考','full evidence ref'),cmp.full_evidence_reference&&cmp.full_evidence_reference.evidence_cost,uiText('证据成本','evidence cost')]
  ].map(r=>'<div><span>'+uiEsc(r[0])+'</span><b>'+uiEsc(r[1]??'-')+'</b><i>'+uiEsc(r[2])+'</i></div>').join('');
  host.innerHTML='<div class="voe-head"><div><span class="voe-kicker">X5-RB-VoE · Risk-Bounded Value of Evidence</span><h2>'+uiText('风险有界证据-动作编译器','Risk-bounded evidence-action compiler')+'</h2><p>'+uiText('不是新增分类器，而是让既有 XRD/PL/BPU/LLM 在失败核、证据独立性、动作成本和权限边界下决定下一项证据。','Not another classifier: existing XRD/PL/BPU/LLM evidence is compiled into the next bounded evidence action under failure-core, independence, cost, and authority constraints.')+'</p></div><div class="voe-state"><b>'+uiEsc(p.decision||'-')+'</b><span>'+uiText('当前离线策略','current offline policy')+'</span></div></div>'+
    '<div class="voe-facts"><div><span>'+uiText('H2 最坏风险','H2 robust risk')+'</span><b>'+uiEsc(p.risk??'-')+'</b><i>'+uiText('HOLD 基线 ','HOLD baseline ')+uiEsc(p.hold_risk??'-')+'</i></div><div><span>'+uiText('证据时域','evidence horizon')+'</span><b>H='+uiEsc(p.horizon??'-')+'</b><i>'+uiText('最大成本 ','max cost ')+uiEsc(p.maximum_evidence_cost??'-')+'</i></div><div><span>'+uiText('执行权限','execution authority')+'</span><b>'+String(a.execution_authority===true)+'</b><i>SIMULATED_COUNTERFACTUAL</i></div><div><span>'+uiText('成熟度','maturity')+'</span><b>'+uiEsc(src.maturity||'-')+'</b><i>'+uiText('离线验收','offline accepted')+'</i></div></div>'+
    '<div class="voe-main"><section class="voe-dag"><div class="voe-section-title"><b>Evidence DAG</b><span>'+uiEsc(dag.minimum_independent_evidence??'-')+' '+uiText('条独立证据','independent evidence')+'</span></div><div class="voe-nodes">'+nodes+'</div><div class="voe-core"><span>Failure core</span>'+((d.failure_core||[]).map(x=>'<b>'+uiEsc(x)+'</b>').join(''))+'</div></section>'+
    '<section class="voe-strategy"><div class="voe-section-title"><b>H2 policy tree</b><span>'+uiEsc(p.reason||'')+'</span></div><div class="voe-root"><span>ROOT</span><b>'+uiEsc(p.root_option_id||'')+'</b></div><div class="voe-branches">'+branches+'</div></section></div>'+
    '<div class="voe-lower"><section class="voe-compare"><div class="voe-section-title"><b>'+uiText('同世界模型对照','Matched-world comparisons')+'</b><span>'+uiText('同策略核重算','recomputed by the same policy core')+'</span></div><div class="voe-compare-grid">'+comparisonRows+'</div></section><section class="voe-hold"><span>HOLD witness</span><b>'+uiEsc(hold.reason||'-')+'</b><p>'+uiText('硬门失败时观测数保持为零；不存在“最接近可行”动作。','A failed hard gate keeps every observation count at zero; no almost-feasible action is executed.')+'</p><i>'+uiText('执行权限缺失 ','authority absent ')+String(hold.execution_authority_absent===true)+'</i></section></div>'+
    '<div class="voe-release"><div><span>RB-VoE release root</span><code title="'+uiEsc(src.release_root_sha256||'')+'">'+uiEsc(rbVoeShortHash(src.release_root_sha256))+'</code></div><div><span>comparison</span><code title="'+uiEsc(src.comparison_sha256||'')+'">'+uiEsc(rbVoeShortHash(src.comparison_sha256))+'</code></div><div><span>public snapshot</span><code title="'+uiEsc(d.public_snapshot_sha256||'')+'">'+uiEsc(rbVoeShortHash(d.public_snapshot_sha256))+'</code></div></div>'+
    '<div class="voe-boundaries"><b>'+uiText('当前证据边界','Current evidence boundary')+'</b>'+(d.boundaries||[]).map(x=>'<p>'+uiEsc(x)+'</p>').join('')+'</div>';
}
...
function cancelDetailRequest(){
  _detailRequestSeq++;
  if(_detailRequest){ _detailRequest.abort(); _detailRequest=null; }
}
function detailOpen(kind,id){
  cancelDetailRequest();
  DETAIL_KIND=kind||'material'; DETAIL_ID=String(id||'').trim(); DETAIL_TAB=DETAIL_KIND==='evidence'?'overview':'structure'; DETAIL_DATA=null;
  if(DETAIL_ID){
    const prefix=DETAIL_KIND==='prediction'?'/predictions/':DETAIL_KIND==='evidence'?'/evidence/':'/materials/';
    const path=prefix+encPath(DETAIL_ID);
    try{ history.pushState({view:'detail',kind:DETAIL_KIND,id:DETAIL_ID},'',path); }catch(e){}
  }
  go('detail',{force:true});
}
async function loadDetail(){
  if(_detailRequest) _detailRequest.abort();
  const controller=new AbortController();
  const requestSeq=++_detailRequestSeq;
  const requestKind=DETAIL_KIND;
  const requestId=DETAIL_ID;
  _detailRequest=controller;
  const title=document.getElementById('dtTitle'), sub=document.getElementById('dtSub'), body=document.getElementById('dtBody');
  if(title) title.textContent=requestKind==='prediction'?uiText('预测详情','Prediction detail'):requestKind==='evidence'?uiText('证据对象','Evidence object'):uiText('材料详情','Material detail');
  ...
  if(!requestId){
    _detailRequest=null;
    detailNotFound(uiText('缺少材料或 trace_id。','Missing material or trace id.'));
    return;
  }
  const prefix=requestKind==='prediction'?'/api/predictions/':requestKind==='evidence'?'/api/evidence_objects/':'/api/materials/';
  const path=prefix+encPath(requestId);
  let timedOut=false;
  ...
  try{
    ...
    if(requestSeq!==_detailRequestSeq || requestKind!==DETAIL_KIND || requestId!==DETAIL_ID) return;
    if(!r.ok){ detailNotFound(d.message||uiText('没有匹配这个 ID 的公开对象。','No public object matched this id.')); return; }
    DETAIL_DATA=d; DETAIL_TAB=requestKind==='evidence'?'overview':'structure'; detailRender();
  }catch(e){
    if(requestSeq!==_detailRequestSeq) return;
    if(e&&e.name==='AbortError'&&!timedOut) return;
    if(timedOut){ detailNotFound(uiText('详情请求超时，请重试。','Detail request timed out. Try again.')); return; }
    detailNotFound(uiText('详情 API 当前不可用。','Detail API is unavailable.'));
  }finally{
    clearTimeout(timeout);
    if(requestSeq===_detailRequestSeq) _detailRequest=null;
  }
}
function detailNotFound(msg){
  ...
  if(badges) badges.innerHTML='<span class="dt-badge off">'+uiText('未找到','not_found')+'</span><span class="dt-badge">release site32-global-commercial-v1.13-20260720</span>';
...
function detailRender(){
  ...
  if(DETAIL_KIND==='evidence'){ detailRenderEvidence(); return; }
...
function detailEvidenceRows(rows){
  return '<div class="dt-grid">'+rows.map(r=>'<div class="dt-row"><span>'+uiEsc(r[0])+'</span><b>'+uiEsc(r[1]==null||r[1]===''?'-':r[1])+'</b></div>').join('')+'</div>';
}

function detailRenderEvidence(){
  const d=DETAIL_DATA||{}, freshness=d.freshness||{}, validation=d.validation||{}, identifier=d.identifier||{};
  const title=document.getElementById('dtTitle'), sub=document.getElementById('dtSub'), badges=document.getElementById('dtBadges');
  const c=document.getElementById('dtCite'), dl=document.getElementById('dtDownloads'), tabs=document.getElementById('dtTabs'), body=document.getElementById('dtBody');
  if(title) title.textContent=uiText(d.title||d.evidence_id||'证据对象',d.title_en||d.title||d.evidence_id||'Evidence object');
  if(sub) sub.textContent=[d.evidence_id,d.scope,d.kind].filter(Boolean).map(uiTerm).join(' · ');
  if(badges) badges.innerHTML=[
    [uiText('来源','source'),d.source_label||freshness.state||'unknown'],
    [uiText('发布','publication'),(d.rights||{}).access||'public-read-only'],
    [uiText('验证','validation'),d.validation_status||validation.result||'unknown'],
    [uiText('结论','claim'),d.claim_status||'unknown']
  ].map(x=>'<span class="dt-badge '+esc(String(x[1]).toLowerCase().replace(/[^a-z0-9_-]+/g,'-'))+'">'+uiEsc(x[0]+': '+uiTerm(x[1]))+'</span>').join('');
  const citation=d.citation||{};
  if(c) c.innerHTML='<b>'+uiText('引用此证据对象','Cite this evidence object')+'</b><code>'+esc(d.evidence_id||'-')+'</code><span>'+uiEsc(uiText(citation.text||d.citation_text||'当前未注册 DOI；使用稳定本地 ID 与发布版本引用。',citation.text_en||d.citation_text_en||'No DOI is registered; cite the stable local ID and release version.'))+'</span>';
  const distributions=Array.isArray(d.distributions)?d.distributions:[];
  if(dl) dl.innerHTML=distributions.map(x=>{
    const href=x.available?detailSafeHref(x.href):'';
    const label=x.label||x.mime||uiText('公开分发','Public distribution');
    return href?'<a class="dt-dl" href="'+esc(href)+'" title="SHA-256 '+esc(x.sha256||'unknown')+'">'+uiEsc(label)+'</a>':'<span class="dt-dl off" title="'+uiEsc(x.availability_reason||x.reason||'Unavailable')+'">'+uiEsc(label)+' '+uiText('不可用','unavailable')+'</span>';
  }).join('');
  const tabTitles={
    overview:uiText('概览','Overview'), evidence:uiText('证据','Evidence'), provenance:uiText('溯源','Provenance'),
    files:uiText('文件','Files'), citation:uiText('引用','Citation'), limits:uiText('限制','Limits')
  };
  if(tabs) tabs.innerHTML=Object.keys(tabTitles).map(k=>'<button class="'+(DETAIL_TAB===k?'on':'')+'" data-dt-tab="'+k+'" onclick="detailSetTab(this.dataset.dtTab)">'+uiEsc(tabTitles[k])+'</button>').join('');
  if(!body) return;
  if(DETAIL_TAB==='overview'){
    const resource=d.resource_type||{};
    body.innerHTML=detailEvidenceRows([
      [uiText('稳定 ID','Stable ID'),d.evidence_id],
      [uiText('对象类型','Object type'),resource.type||d.kind],
      [uiText('作用域','Scope'),d.scope],
      [uiText('发布日期','Published'),(d.dates||{}).published],
      [uiText('快照时间','Snapshot time'),freshness.checked_at],
      [uiText('来源置信','Source confidence'),freshness.confidence],
      [uiText('DOI','DOI'),identifier.doi||d.doi||uiText('未注册','not registered')],
      [uiText('许可','License'),(d.rights||{}).license],
      [uiText('用途','Intended use'),uiText(d.intended_use,d.intended_use_en||d.intended_use)],
      [uiText('禁止解释','Prohibited interpretation'),uiText(d.prohibited_use,d.prohibited_use_en||d.prohibited_use)]
    ])+'<div class="dt-note">'+uiEsc(uiText(d.claim||d.description||'',d.description_en||d.claim||d.description||''))+'</div>';
    return;
  }
  if(DETAIL_TAB==='evidence'){
    const claims=Array.isArray(d.claims)&&d.claims.length?d.claims:[{statement:d.claim,status:d.claim_status,source_label:d.source_label,evidence_modes:d.origin||[]}];
    const checks=Array.isArray(validation.checks)?validation.checks:[];
    body.innerHTML='<div class="dt-evidence-list">'+claims.map(x=>'<article><div><span>'+uiEsc(x.source_label||d.source_label||'unknown')+'</span><b>'+uiEsc(x.status||d.claim_status||'unknown')+'</b></div><p>'+uiEsc(uiText(x.statement||'',x.statement_en||x.statement||''))+'</p><i>'+uiEsc((x.evidence_modes||[]).join(' · '))+'</i></article>').join('')+
      (checks.length?'<article><div><span>'+uiText('验证检查','Validation checks')+'</span><b>'+uiEsc(validation.result||d.validation_status||'unknown')+'</b></div><p>'+uiEsc(checks.join(' · '))+'</p><i>'+uiEsc(uiText(validation.scope||'',validation.scope_en||validation.scope||''))+'</i></article>':'')+'</div>';
    return;
  }
  if(DETAIL_TAB==='provenance'){
    const provenance=d.property_provenance||{};
    const rows=Object.keys(provenance).map(k=>[k,Object.entries(provenance[k]||{}).map(x=>x[0]+'='+x[1]).join(' · ')]);
    const relations=Array.isArray(d.relations)?d.relations:[];
    body.innerHTML=detailEvidenceRows(rows)+'<div class="dt-relation-list">'+relations.map(x=>{
      const href=detailSafeHref(x.target);
      const target=href?'<a href="'+esc(href)+'">'+uiEsc(x.target||'')+'</a>':'<code>'+uiEsc(x.target||'')+'</code>';
      return '<div><span>'+uiEsc(x.relation_type||'related')+'</span><b>'+uiEsc(x.target_kind||'object')+'</b>'+target+'</div>';
    }).join('')+'</div>';
    return;
  }
  if(DETAIL_TAB==='files'){
    body.innerHTML='<div class="dt-file-list">'+(distributions.length?distributions.map(x=>'<article><div><b>'+uiEsc(x.label||'Distribution')+'</b><span>'+uiEsc(x.available?uiText('可用','available'):uiText('不可用','unavailable'))+'</span></div>'+detailEvidenceRows([[uiText('MIME','MIME'),x.mime||x.mime_type],[uiText('字节','Bytes'),x.bytes],[uiText('SHA-256','SHA-256'),x.sha256],[uiText('许可','License'),x.license]])+'</article>').join(''):'<div class="dt-note">'+uiText('没有公开分发文件。','No public distributions.')+'</div>')+'</div>';
    return;
  }
  if(DETAIL_TAB==='citation'){
    body.innerHTML='<div class="dt-citation-stack"><section><b>'+uiText('纯文本','Plain text')+'</b><code>'+uiEsc(uiText(citation.text||d.citation_text||'',citation.text_en||d.citation_text_en||citation.text||d.citation_text||''))+'</code></section><section><b>BibTeX</b><code>'+uiEsc(uiText(citation.bibtex||'',citation.bibtex_en||citation.bibtex||''))+'</code></section><section><b>RIS</b><code>'+uiEsc(uiText(citation.ris||'',citation.ris_en||citation.ris||''))+'</code></section></div>';
    return;
  }
  const limits=uiText(Array.isArray(d.limitations)?d.limitations:[],Array.isArray(d.limitations_en)?d.limitations_en:(Array.isArray(d.limitations)?d.limitations:[]));
  const excluded=((d.redaction||{}).excluded||[]);
  body.innerHTML='<div class="dt-limit-stack"><section><b>'+uiText('已知限制','Known limitations')+'</b><ul>'+limits.map(x=>'<li>'+uiEsc(x)+'</li>').join('')+'</ul></section><section><b>'+uiText('不确定性','Uncertainty')+'</b><p>'+uiEsc(uiText((d.uncertainty||{}).statement||'unknown',(d.uncertainty||{}).statement_en||(d.uncertainty||{}).statement||'unknown'))+'</p></section><section><b>'+uiText('公开脱敏','Public redaction')+'</b><p>'+uiEsc(excluded.join(' · ')||uiText('未声明','not declared'))+'</p></section></div>';
}
  ...
  try{
    ...
    if(p){
      ...
      if(parts[0]==='evidence' && parts[1]){
        DETAIL_KIND='evidence'; DETAIL_ID=decodeURIComponent(parts.slice(1).join('/')); DETAIL_TAB='overview'; DETAIL_DATA=null;
        return 'detail';
      }
...
const AR_ROAD=[
  ...
  {d:'2026-07', t:'双机械臂复赛闭环', s:'arm01 单臂视觉冗余与投袋 + arm02 并发四周期研磨 + X5 CPU/OpenCV 权威门控 + BPU 辅助证据'},
  {d:'2026-06', t:'公网平台 + 工业级全栈 + UI 升级轮', s:'CF+VPS 双保险 + SSO + historian/ISA-18.2 告警/ISA-88 工单/SLO + 数字孪生 + 指挥中心门户'},
];
function arTab(t){ AR_TAB=t;
  document.querySelectorAll('.ar-tab').forEach(x=>{
    const active=x.dataset.t===t; x.classList.toggle('on',active); x.setAttribute('aria-selected',active?'true':'false');
  }); loadArchive(); }
...
const STD_WALL=[
  ...
  {std:'ISA-88', area:'批记录', impl:'批次工单生命周期: 创建即调 AI 预测并绑定 trace_id/verdict → 路径/工位证据记录 → 烧结/表征 → 实测回填收单; 每步落 wo_log 记操作人, 批次档案可导出', go:'mq', goT:'批次工单'},
  {std:'21 CFR Part 11 principles', area:'电子记录/审计追溯', impl:'预测记录 SHA-256 hash 链 + 登录审计 logins.jsonl + RBAC (admin/member/judge) + 操作人留痕；当前为工程思想映射，非认证', go:'ops', goT:'运维 → 审计'},
...
let _arLoaded=false, _arStatusBound=false;
function loadAR(){
  ...
  if(mv){
    ...
    if(!_arStatusBound){
      ...
      mv.addEventListener('ar-status',e=>{
        const current=document.getElementById('arStat');
        if(e.detail&&e.detail.status==='session-started'&&current) current.textContent='AR 进行中';
      });
    }
...
const OBS={metric:'latency',range:'6h',data:null,timer:null,resizeBound:false};
...
function loadObs(){
  ...
  if(!OBS.resizeBound){ window.addEventListener('resize', obsRender); OBS.resizeBound=true; }
}
...
async function loadFleetConsole(){
  ...
  const ts=document.getElementById('fleetConsoleTs'); if(ts) ts.textContent='release '+(d.release||'')+' · '+new Date((d.ts||Date.now()/1000)*1000).toLocaleTimeString('zh-CN');
  const s=d.summary||{};
...
async function loadTasksConsole(){
  ...
  rows.forEach((t,index)=>{
    ...
    h+='<article class="r8-task '+(t.manual_review?'review':'')+'">'+
      ...
      (t.trace_id?'<button type="button" class="i-mini" data-task-trace="'+index+'">'+uiText('打开链路','Open trace')+'</button>':'')+
      '<div class="r8-history">'+(hist||'<span class="r8-emptyline">'+uiText('暂无近期历史','no recent history')+'</span>')+'</div></article>';
  });
  h+='</div>';
  if(b){
    b.innerHTML=h;
    b.querySelectorAll('[data-task-trace]').forEach(button=>{
      const index=Number(button.dataset.taskTrace);
      button.addEventListener('click',()=>{
        const item=rows[index], traceId=String(item&&item.trace_id||'');
        if(traceId) go('traces',{after:()=>traceFill(traceId)});
      });
    });
  }
}
...
function logsNode(tag,cls,text){
  const node=document.createElement(tag);
  if(cls) node.className=cls;
  if(text!=null) node.textContent=String(text);
  return node;
}
function logsEmpty(host,message){
  if(!host) return;
  host.replaceChildren(logsNode('div','oe-empty',message));
}
function logsSourceNode(source){
  const raw=String(source||'unknown');
  return logsNode('span','r8-src '+r8Cls(raw),uiTerm(raw));
}
function logsCell(row,child,cls){
  const cell=logsNode('td',cls||'');
  if(child instanceof Node) cell.appendChild(child);
  else cell.textContent=child==null?'':String(child);
...
async function logsFetch(){ const g=id=>{const e=document.getElementById(id);return e?e.value:'';};
  ...
  }catch(e){ logsEmpty(body,uiText('日志获取失败','Unable to load logs')); return; }
  const ts=document.getElementById('logsTs'); if(ts) ts.textContent='环形日志 · 库内 '+d.total+' 行 · 显示 '+d.n;
  if(!body) return;
  const wrap=logsNode('div','i-table logs-t');
  const table=logsNode('table');
  const head=logsNode('thead'), headRow=logsNode('tr');
  ['时间','服务','方法','路由','状态','延迟','用户','IP',''].forEach(label=>headRow.appendChild(logsNode('th','',uiTerm(label))));
  head.appendChild(headRow); table.appendChild(head);
  const tbody=logsNode('tbody');
  const entries=Array.isArray(d.logs)?d.logs:[];
  entries.forEach(l=>{
    const rid=String(l&&l.req_id||'');
    const status=Number(l&&l.status);
    const sc=status>=500?'crit':status>=400?'warn':'ok';
    const row=logsNode('tr','lg-row');
    row.tabIndex=0;
    row.setAttribute('aria-label',uiText('打开请求追踪 ','Open request trace ')+(rid||uiText('未提供标识','without an id')));
    const openTrace=()=>{ if(rid) logTrace(rid); };
    row.addEventListener('click',openTrace);
    row.addEventListener('keydown',event=>{
      if(event.target!==row) return;
      if(event.key==='Enter'||event.key===' '){ event.preventDefault(); openTrace(); }
    });
    logsCell(row,fmtClock(l&&l.ts).slice(6));
    logsCell(row,logsSourceNode(l&&l.service));
    const method=String(l&&l.method||'—');
    logsCell(row,logsNode('span','mlab '+r8Cls(method),method));
    logsCell(row,logsNode('code','',l&&l.route||''));
    logsCell(row,logsNode('span','sev '+sc,l&&l.status==null?'—':l.status));
    logsCell(row,l&&l.ms==null?'—':String(l.ms)+'ms');
    logsCell(row,l&&l.usr||'');
    logsCell(row,l&&l.ip||'','i-dim');
    const link=logsNode('button','i-mini',uiText('链接','link'));
    link.type='button'; link.disabled=!rid;
    link.setAttribute('aria-label',uiText('复制追踪链接 ','Copy trace link ')+rid);
    link.addEventListener('click',event=>{ event.stopPropagation(); if(rid) copyTraceLink(rid); });
    logsCell(row,link);
    tbody.appendChild(row);
  });
  if(!entries.length){
    const row=logsNode('tr'), cell=logsNode('td','');
    cell.colSpan=9;
    cell.appendChild(logsNode('div','oe-empty',uiText('无匹配日志; 过滤条件不会暴露 token 或密钥','No matching logs; filters never expose tokens or secrets')));
    row.appendChild(cell); tbody.appendChild(row);
  }
  table.appendChild(tbody); wrap.appendChild(table); body.replaceChildren(wrap);
}
async function logTrace(rid){
  rid=String(rid||'');
  if(!rid) return;
  let d; try{
    const response=await fetch('/api/trace/'+encodeURIComponent(rid),{cache:'no-store'});
    if(!response.ok) throw new Error('trace '+response.status);
    d=await response.json();
  }catch(e){ toast(uiText('请求追踪获取失败','Unable to load request trace'),'warn'); return; }
  iModalOpen('');
  const modal=document.getElementById('iModalBody');
  if(!modal) return;
  const fragment=document.createDocumentFragment();
  const title=logsNode('h3','',uiText('请求追踪 ','Request trace '));
  title.appendChild(logsNode('code','',rid)); fragment.appendChild(title);
  const list=logsNode('div','trace-list');
  const spans=Array.isArray(d.spans)?d.spans:[];
  spans.forEach(s=>{
    const status=Number(s&&s.status);
    const sc=status>=500?'crit':status>=400?'warn':'ok';
    const span=logsNode('div','trace-sp');
    const method=String(s&&s.method||'—');
    span.appendChild(logsNode('span','mlab '+r8Cls(method),method));
    span.appendChild(logsSourceNode(s&&s.service));
    span.appendChild(logsNode('code','',s&&s.route||''));
    span.appendChild(logsNode('span','sev '+sc,s&&s.status==null?'—':s.status));
    span.appendChild(logsNode('b','',s&&s.ms==null?'—':String(s.ms)+'ms'));
    span.appendChild(logsNode('span','i-dim',fmtClock(s&&s.ts).slice(6)+' · '+String(s&&s.usr||'')+' · '+String(s&&s.ip||'')));
    list.appendChild(span);
  });
  if(!spans.length) list.appendChild(logsNode('div','oe-empty',uiText('无追踪记录','No trace records')));
  fragment.appendChild(list);
  const copy=logsNode('button','i-mini',uiText('复制追踪链接','Copy trace link'));
  copy.type='button'; copy.addEventListener('click',()=>copyTraceLink(rid)); fragment.appendChild(copy);
  fragment.appendChild(logsNode('div','hl-foot',uiText('同一 req_id 关联该请求的全部 span; IP 已脱敏。','The same req_id links every span for this request; IP addresses are masked.')));
...
async function loadTracesConsole(){
  ...
  rows.forEach((t,index)=>{
    h+='<article class="r8-trace"><div class="r8-head"><div><b><code>'+esc(t.trace_id)+'</code></b><span>'+uiEsc(t.kind)+' · '+uiEsc(t.formula||t.work_order||'request')+'</span></div>'+r8Src(t.source)+'</div>'+
      ...
      traceWaterfall(t.waterfall||[])+'<button type="button" class="i-mini" data-trace-copy="'+index+'">'+uiText('复制分享链接','copy share link')+'</button></article>';
  });
  h+='</div><div class="hl-foot">'+uiEsc(d.note||'')+'</div>';
  if(b){
    b.innerHTML=h;
    b.querySelectorAll('[data-trace-copy]').forEach(button=>{
      const index=Number(button.dataset.traceCopy);
      button.addEventListener('click',()=>{ const item=rows[index]; if(item) copyTraceLink(String(item.trace_id||'')); });
    });
  }
}
...
async function coaShow(){ const b=_qmsBatch(); if(!b){ toast('请输入工单号'); return; } let d; try{ d=await fetch('/api/coa/'+encodeURIComponent(b),{cache:'no-store'}).then(r=>r.json()); }catch(e){ return; } if(d.error){ toast(d.error); return; }
  ...
  h+='<div class="coa-sign"><div class="coa-sh">项目电子签批</div>';
...
async function esignDo(ot,oid){ const reason=prompt('签批原因 (项目审计留痕):','批次放行 approved'); if(reason==null) return; try{ await iPost('/api/esign',{obj_type:ot,obj_id:oid,meaning:'approved',reason:reason}); toast('已电子签批'); coaShow(); }catch(e){} }
...
const SEC_ZH={
  'Admin':'管理员','Member':'成员','Judge / Demo':'评委 / 演示','web admin':'Web 管理端','web member':'Web 成员端','judge':'评委端',
  'configuration, rules, release view':'配置、规则与发布查看','prediction, experiment feedback and maintenance records':'预测、实验回填与维保记录','read-only demo mode':'只读演示模式',
  'manage config':'管理配置','create alert rules':'创建告警规则','view admin audit':'查看管理审计','create prediction':'创建预测','submit actual feedback':'提交实测回填','record maintenance':'记录维保','open demo pages':'打开演示页面','inspect public evidence':'复核公开证据',
  'login':'登录','prediction':'预测','actual_feedback':'实测回填','robot_command':'机器人操作留痕','deployment':'发布','settings_change':'设置变更','permission_change':'权限变更','api_key_action':'API 密钥操作',
  'Public read-only method boundary':'公网只读方法边界','Host allowlist':'Host 白名单','Request body limit':'请求体上限','Security headers':'浏览器安全响应头','Origin service binding':'Origin 服务绑定','VPS firewall':'VPS 防火墙','Cloudflare WAF managed rules':'Cloudflare WAF 托管规则','Cloudflare rate limiting':'Cloudflare 速率限制','Rollback and release records':'回滚与发布记录',
  'Managed Rules / OWASP ruleset / HTTP DDoS':'托管规则 / OWASP 规则集 / HTTP DDoS 防护','Cache static assets; keep dynamic pages DYNAMIC/no-cache':'缓存静态资源；动态页面保持不缓存','Rate limit /api/*, /api/openapi.json, /api/global_benchmark, /api/hardening, /api/evidence_bundle.json':'为公开 API 家族配置速率限制','Block abnormal methods and common scanner paths at edge':'在边缘阻断异常方法与常见扫描路径','Authenticated Origin Pulls / Access for SSH-admin flows':'认证源站回源 / 管理访问隔离',
  'Versioned static assets':'静态资源版本化','Service worker cache':'Service Worker 缓存','Three.js route throttling':'Three.js 路由节流','Offline API cache whitelist':'离线 API 缓存白名单','Machine-readable API catalog':'机器可读 API 目录','Public evidence manifest':'公开证据清单','Research passport':'科研护照','Site31 evidence contract':'Site31 证据契约','Evidence bundle download':'证据包下载','Global benchmark console':'全球对标控制台','SEO discovery files':'SEO 发现文件','Public unsafe-control scan':'公网危险控制扫描',
  'Language / skip link / landmarks':'语言、跳转链接与地标','Accessible names / unique IDs':'控件可访问名称与唯一 ID','Focus visible / current route':'可见焦点与当前路由','Reduced motion / transparency':'减少动态与透明效果','Polite route status announcement':'路由状态礼貌播报','Keyboard / zoom browser matrix':'键盘与缩放浏览器矩阵','NVDA / VoiceOver / WCAG-EM':'NVDA / VoiceOver / WCAG-EM 人工审计'
};
const OWASP_ZH={A01:'访问控制失效',A02:'加密机制失效',A03:'注入',A04:'不安全设计',A05:'安全配置错误',A06:'易受攻击和过时组件',A07:'身份识别与认证失效',A08:'软件与数据完整性失效',A09:'安全日志与监控失效',A10:'服务端请求伪造'};
function secZh(v){ const s=String(v??''); return SEC_ZH[s]||s; }
function secStateLabel(v){ const s=String(v||'unknown').toLowerCase(); const zh={verified:'已验证','verified-partial':'门禁通过 · 保留人工项','implemented-partial':'已实现 · 证据待补','manual-check':'需人工核验',planned:'计划中',failed:'失败',missing:'缺失',tracked:'已留痕',masked:'已脱敏',gateway:'网关验证',unavailable:'暂不可用',pass:'通过',warn:'待核验',future:'后续',must:'必须',should:'建议',optional:'可选'}; return uiText(zh[s]||s,s); }
function secStates(rows){ return (rows||[]).map(x=>secStateLabel(x.state)).join(' / '); }
function secVerified(v){ return ['pass','verified','tracked','masked','gateway'].includes(String(v||'').toLowerCase()); }
async function secFetch(){
  ...
  const base={ts:Date.now()/1000, score:0, pass:0, total:0, fail:0, checks:[], roles:{},
    ...
    note:uiText('先加载 Site31 控制清单，再分别补入运行态与网关证据。','Site31 control inventory is loaded first; runtime and gateway evidence fill in separately.')};
  try{
    h9=await fetch('/api/hardening',{cache:'no-store'}).then(r=>r.json());
    const gate=h9.gate_evidence||{}, dims=gate.dimensions||{};
    const localChecks=[
      {name:uiText('RBAC 角色目录','RBAC role catalog'), status:(h9.roles||[]).length===3?'pass':'warn', detail:uiText(((h9.roles||[]).length||0)+' 类已部署角色',((h9.roles||[]).length||0)+' deployed role classes')},
      {name:uiText('审计覆盖矩阵','Audit coverage matrix'), status:'warn', detail:uiText(((h9.audit||[]).length||0)+' 类事件已记录 · 仍需运行态复核',((h9.audit||[]).length||0)+' documented event classes · runtime review required')},
      {name:uiText('OWASP Top 10 复核','OWASP Top 10 review'), status:'warn', detail:uiText(((h9.owasp||[]).length||0)+' 行内部复核 · 不是渗透测试',((h9.owasp||[]).length||0)+' review rows · not a penetration test')},
      {name:uiText('性能与缓存检查','Performance checks'), status:(h9.performance||[]).every(x=>!['failed','missing'].includes(x.state))?'pass':'warn', detail:secStates(h9.performance)},
      {name:uiText('可访问性检查','Accessibility checks'), status:gate.valid&&Number(dims.accessibility?.ratio||0)>=.75?'pass':'warn', detail:secStates(h9.accessibility)},
      {name:uiText('公网边界分层','Public boundary layers'), status:gate.valid&&Number(dims.security?.ratio||0)>=.75?'pass':'warn', detail:secStates(h9.hardening_layers)},
      {name:uiText('发布门禁证据','Release gate evidence'), status:gate.valid&&gate.gate==='pass'?'pass':'warn', detail:(gate.phase||'missing')+' · '+(gate.artifact_sha256||'').slice(0,12)},
      {name:uiText('回滚路径','Rollback path'), status:h9.rollback?'pass':'warn', detail:h9.release||uiText('本地加固','local hardening')}
    ];
...
function r9State(s){ s=String(s||'unknown').toLowerCase(); return secVerified(s)?'ok':(s==='fail'||s==='failed'||s==='missing'?'crit':'warn'); }
...
function r9MiniRows(rows, cols){
  return '<div class="r9-mini">'+(rows||[]).map(x=>'<div class="r9-mini-row">'+cols.map(c=>'<span>'+esc(c==='state'?secStateLabel(x[c]):secZh(x[c]||'—'))+'</span>').join('')+'</div>').join('')+'</div>';
}
function secRender(d){
  ...
  let h='<div class="sec-top"><div class="sec-score '+tone+'"><div class="sec-sl">已验证检查比率</div><div class="sec-sv">'+d.score+'</div><div class="sec-ss">'+d.pass+'/'+d.total+' 已验证 · '+d.fail+' 未验证或待改进</div></div>';
  ...
  if(h9.release){
    h+='<div class="i-sec"><div class="i-sh">'+uiText('Site31 Trust 控制 · RBAC 角色','Site31 Trust Controls · RBAC roles')+'</div><div class="r9-role-grid">'+(h9.roles||[]).map(r=>
      '<article class="r9-role"><div><b>'+esc(uiText(secZh(r.label),r.label))+'</b><span>'+esc(uiText(secZh(r.ui),r.ui))+'</span></div><p>'+esc(uiText(secZh(r.scope),r.scope))+'</p><em>'+esc((r.public_actions||[]).map(x=>uiText(secZh(x),x)).join(' · '))+'</em></article>'
    ).join('')+'</div></div>';
    h+='<div class="i-sec"><div class="i-sh">'+uiText('审计事件覆盖','Audit event coverage')+'</div><div class="r9-audit">'+(h9.audit||[]).map(a=>
      '<div class="r9-audit-row '+r9State(a.state)+'"><b>'+esc(uiText(secZh(a.event),a.event))+'</b><span>'+esc(a.source)+'</span><strong>'+esc(a.records)+'</strong><em>'+esc(secStateLabel(a.state))+'</em></div>'
    ).join('')+'</div></div>';
    h+='<div class="i-sec"><div class="i-sh">'+uiText('OWASP Top 10 内部复核','OWASP Top 10 review')+'</div><div class="r9-owasp">'+(h9.owasp||[]).map(o=>
      '<div class="r9-ow '+r9State(o.state)+'"><span>'+esc(o.key)+'</span><b>'+esc(uiText(OWASP_ZH[o.key]||o.name,o.name))+'</b><em>'+esc(secStateLabel(o.state))+'</em><p>'+esc(o.evidence)+'</p></div>'
    ).join('')+'</div></div>';
    h+='<div class="i-sec r9-two"><div><div class="i-sh">'+uiText('性能 / 缓存','Performance / cache')+'</div>'+r9MiniRows(h9.performance||[],['name','state','detail'])+'</div><div><div class="i-sh">'+uiText('可访问性','Accessibility')+'</div>'+r9MiniRows(h9.accessibility||[],['name','state'])+'</div></div>';
    h+='<div class="i-sec"><div class="i-sh">Site31 分层防护状态</div><div class="r9-owasp">'+(h9.hardening_layers||[]).map(o=>
      '<div class="r9-ow '+r9State(o.state)+'"><span>'+esc(o.layer)+'</span><b>'+esc(uiText(secZh(o.name),o.name))+'</b><em>'+esc(secStateLabel(o.state))+'</em><p>'+esc(o.detail)+'</p></div>'
    ).join('')+'</div></div>';
    h+='<div class="i-sec"><div class="i-sh">Cloudflare WAF / 限速配置清单</div><div class="r9-audit">'+(h9.cloudflare_manual||[]).map(a=>
      '<div class="r9-audit-row '+r9State(a.status)+'"><b>'+esc(secStateLabel(a.priority))+'</b><span>'+esc(uiText(secZh(a.item),a.item))+'</span><strong>'+esc(secStateLabel(a.status))+'</strong><em>edge</em></div>'
    ).join('')+'</div></div>';
    h+='<div class="i-sec"><div class="i-sh">'+uiText('静态资源预算','Static asset budget')+'</div><div class="r9-assets">'+(h9.assets||[]).map(a=>'<span>'+esc(a.name)+' <b>'+r9Bytes(a.bytes)+'</b></span>').join('')+'</div><div class="hl-foot">'+uiText('回滚路径','Rollback path')+': <code>'+esc(h9.rollback||'')+'</code></div></div>';
  }
  h+='<div class="hl-foot" style="margin-top:14px">'+esc(d.note||'')+'</div>';
...
/* ---- I10 i18n 中英双语 ---- */
const I18N_DEFAULT_VER='site32-global-commercial-v1.13-20260720';
...
function i18nToggle(){
  const currentLang=(localStorage.getItem('xrd_lang')||'zh');
  const nextLang=currentLang==='zh'?'en':'zh';
  localStorage.setItem('xrd_lang',nextLang);
  localStorage.setItem('xrd_lang_default_ver',I18N_DEFAULT_VER);
  applyI18n(nextLang);
  if(_lastPublicStatus) psRender(_lastPublicStatus);
  if(_researchCollections.length) researchCommonsRender();
  applyAudienceRole(window._accessRole||'public');
  if(DETAIL_DATA && document.body.dataset.view==='detail') detailRender();
  document.querySelectorAll('#overview .scene-hint').forEach(x=>x.textContent=uiText('拖动旋转 · 真实感实验室数字孪生','Drag to rotate · realistic lab digital twin'));
  toast(nextLang==='en'?'English':'中文');
}
...
function applyI18n(lang){ lang=lang||localStorage.getItem('xrd_lang')||'zh'; const dict=(window.I18N&&window.I18N[lang])||{};
  ...
  if(b){ b.textContent=lang==='en'?'EN/中':'中/EN'; b.setAttribute('aria-label',lang==='en'?'Language switch':'语言切换'); }
  const chromeA11y=lang==='en'
    ? {bell:['Notification center','Notifications (alerts, events and milestones)'],btnTour:['Start demo','Start demo'],btnMore:['More menu','More (pages and settings)']}
    : {bell:['通知中心','通知中心 (告警 · 事件 · 大事记)'],btnTour:['开始演示','开始演示'],btnMore:['更多菜单','更多 (功能页 · 设置)']};
  Object.entries(chromeA11y).forEach(([id,label])=>{ const el=document.getElementById(id); if(el){ el.setAttribute('aria-label',label[0]); el.setAttribute('title',label[1]); } });
  const primaryNav=document.getElementById('iaStrip'); if(primaryNav) primaryNav.setAttribute('aria-label',lang==='en'?'Research portal workspaces':'科研门户工作域');
...
const GLOSSARY=[
  ...
  {cat:'机器人', t:'AprilTag', d:'类似二维码的视觉基准标记, 用于精确估计相机相对工件/工位的 6 自由度位姿。机械臂工位用它做标定。'},
...
const CHANGELOG=[
  ...
  {d:'2026-07', tag:'里程碑', t:'具身脑 + 双机械臂复赛闭环', items:['取瓶、升顶、0.50m 里程计闭环、下降放瓶与复位真机完成','SLAM 建图链路 + Lab-FSD shadow/assist 口径','arm01 视觉冗余与投袋 + arm02 并发四周期研磨真机完成']},
...
setInterval(()=>{ if(!document.hidden&&cur==='ops') pollOps(); }, 10000);
...
const TOUR=[
  ...
  { sys:'car', route:'/livemap',  n:'02', t:'车载脑 · SLAM / shadow', live:'car', bullets:['AI 脑下发取料派单','SLAM 建图证据 + Lab-FSD shadow 路径','Nav2/MPPI 与安全员保持执行边界'] },
  { sys:'arm', route:'/pipeline', n:'03', t:'机械臂 · 双臂协同', live:'arm', bullets:['arm01 单臂视觉冗余与投袋真机完成','arm02 并发四周期研磨真机完成','CPU/OpenCV 为袋状态权威，BPU 仅作辅助与执行证据'] },
  { sys:'lab', route:'/campaign', n:'04', t:'AI 脑 · 实测回填', live:'kpi', bullets:['烧结/XRD/PL 实测结果回填','GP/EI 闭环主动学习','驱动下一轮配方, 闭环实验飞轮'] },
];
...
async function theaterLive(kind){
  ...
  try{
    if(kind==='pred'){
      ...
      if(k.audit_intact) bits.push('审计链 '+k.audit_valid+'/'+k.audit_total+' 当前校验完整');
      ...
      const srcLabel=sceneTelemetrySourceLabel({source:src});
      if(kind==='car'&&d.car){ const b=d.car.battery_pct; const v=(d.car.velocity||{});
        out='车载脑'+srcLabel+' · '+(b!=null?('电量 '+b+'% · '):'')+'位姿/路线证据 · source='+srcLabel; }
      else if(kind==='arm'){ const a=(d.arms||{}).arm01||(d.arms||{}).arm02;
        if(a&&a.angles) out='arm01/工位关节角 [' + a.angles.slice(0,3).map(x=>x.toFixed(0)).join(', ')+'…] J · '+
          'source='+srcLabel; }
...
function cancelTheaterSceneTimers(){
  (TH.sceneTimers||[]).forEach(clearTimeout);
  TH.sceneTimers=[];
}
function sendScene(sys, route, sceneSeq){
  ...
  cancelTheaterSceneTimers();
  [0,700,1500].forEach(d=>TH.sceneTimers.push(setTimeout(()=>{
    if(!TH.on || sceneSeq!==TH.enterSeq) return;
    try{ f.contentWindow.postMessage(msg, origin); }catch(e){}
  }, d)));
}
const SYSLABEL={lab:'AI 脑',car:'车载脑',arm:'机械臂'};
let TH={on:false, phase:'idle', idx:0, playing:true, start:Infinity, paused:0, pauseAt:0, dur:9500, raf:0, guard:0, enterSeq:0, sceneTimers:[]};
function tourStart(){
  onbEnd();
  clearTimeout(TH.guard); cancelAnimationFrame(TH.raf); cancelTheaterSceneTimers();
  TH.enterSeq++; _thLiveSeq++;
  TH.on=true; TH.phase='entering'; TH.idx=0; TH.playing=true;
...
function theaterStageReady(seq, s, fallback){
  if(seq!==TH.enterSeq || !TH.on || !['entering','paused-awaiting-ready'].includes(TH.phase)) return;
  ...
  if(TH.playing){ TH.phase='playing'; TH.pauseAt=0; }
  else { TH.phase='paused'; TH.pauseAt=TH.start; }
  sendScene(s.sys, s.route, seq);
}
function theaterEnter(){
  const s=TOUR[TH.idx];
  const seq=++TH.enterSeq;
  clearTimeout(TH.guard); cancelTheaterSceneTimers(); _thLiveSeq++;
  TH.phase=TH.playing?'entering':'paused-awaiting-ready';
  ...
  prev.disabled=TH.idx===0; prev.setAttribute('aria-disabled',prev.disabled?'true':'false');
  // H1 真联动: 拉本幕实时数据进字幕
...
function theaterTick(now){
  ...
  if(TH.playing) TH.raf=requestAnimationFrame(theaterTick);
}
function theaterNext(){
  if(TH.idx>=TOUR.length-1){ TH.playing=false; updatePlay();
    TH.phase='completed';
    cancelAnimationFrame(TH.raf); TH.raf=0;
...
function theaterPlayPause(){
  if(TH.playing){
    TH.playing=false; TH.pauseAt=performance.now();
    TH.phase=TH.start===Infinity?'paused-awaiting-ready':'paused';
    cancelAnimationFrame(TH.raf); TH.raf=0;
  }else{
    if(TH.start!==Infinity && TH.pauseAt) TH.paused += performance.now()-TH.pauseAt;
    TH.playing=true; TH.phase=TH.start===Infinity?'entering':'playing';
    if(!TH.raf) TH.raf=requestAnimationFrame(theaterTick);
  }
  updatePlay();
}
function updatePlay(){ const b=document.getElementById('thPlay');
  b.textContent = TH.playing ? '⏸ 暂停' : '▶ 播放';
  b.setAttribute('aria-pressed',TH.playing?'true':'false'); }
function tourExit(){ TH.on=false; TH.phase='exiting'; cancelAnimationFrame(TH.raf); TH.raf=0;
  clearTimeout(TH.guard); cancelTheaterSceneTimers(); TH.enterSeq++; _thLiveSeq++;
...
const ONB=[
  ...
  { sel:'#btnTour', title:'▶ 一条龙演示', body:'播放端到端证据剧场: 配方预测 → 路径/工位回放 → 表征回填。', pad:8 },
...
function lg3MotionOff(){
  const mode=document.documentElement.dataset.site32PresentationMode||'vivid';
  return mode!=='vivid' || !!(window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches);
}
function lg3Apply(){
  LG3.raf=0;
...
function lg3Reset(){
  if(LG3.raf){ cancelAnimationFrame(LG3.raf); LG3.raf=0; }
  LG3.tx=LG3.ty=LG3.sx=LG3.sy=LG3.scroll=LG3.glx=LG3.gly=LG3.lx=LG3.ly=0;
  lg3Apply();
}
...
window.addEventListener('site32:appearance-change',()=>{ if(lg3MotionOff()) lg3Reset(); });
...
function r4RenderTier(){
  try{
    if(window.R4Performance && window.R4Performance.getTier) return window.R4Performance.getTier();
  }catch(e){}
  return (document.body&&document.body.dataset.r4Render)||'balanced';
}
function initLivingBackground(){
  ...
  const S={mode:'home', wings:[], symbols:[], parts:[], raf:0, w:0, h:0, dpr:1, hidden:false, active:true, reduce:motionReduced(), renderTier:r4RenderTier(), last:0};
  function presentationMode(){
    const mode=document.documentElement.dataset.site32PresentationMode;
    return mode==='defense'||mode==='minimal'?mode:'vivid';
  }
  function animated(){ return presentationMode()==='vivid'&&!S.reduce; }
  function frameTime(){ return animated()?performance.now():0; }
  S.active=presentationMode()!=='minimal';
  function makeWings(){
  ...
  function resize(){
    S.dpr=Math.min(window.devicePixelRatio||1,S.renderTier==='balanced'?1.25:1);
    ...
    placeWings(S.mode);
    placeSymbols(S.mode);
    if(S.active) draw(frameTime());
  }
  function buildParticles(){
    const r=lbgRand(lbgSeed('p-'+S.mode+'-'+Math.round(S.w/100)+'-'+Math.round(S.h/100)));
    const n=S.renderTier==='static'?0:(S.reduce?24:(S.w<760?48:118));
    S.parts=[];
  ...
  function loop(t){
    S.raf=0;
    if(S.hidden||!animated()||!S.active||S.renderTier!=='balanced') return;
    if(S.last&&t-S.last<33){ S.raf=requestAnimationFrame(loop); return; }
    S.last=t;
    draw(t);
    S.raf=requestAnimationFrame(loop);
  }
  function kick(){ if(!S.raf&&!S.hidden&&animated()&&S.active&&S.renderTier==='balanced') S.raf=requestAnimationFrame(loop); }
  window.livingSetView=function(k){
    const mode=lbgModeFor(k);
    S.active=presentationMode()!=='minimal';
    if(mode===S.mode){ setCssMode(mode); if(S.active) draw(frameTime()); kick(); return; }
    S.mode=mode;
    setCssMode(mode);
    placeWings(mode);
    placeSymbols(mode);
  ...
  document.addEventListener('visibilitychange',()=>{
    ...
    if(S.hidden&&S.raf){ cancelAnimationFrame(S.raf); S.raf=0; }
    if(!S.hidden) kick();
  });
  window.addEventListener('r4renderchange',e=>{
    const next=(e.detail&&e.detail.tier)||r4RenderTier();
    if(next===S.renderTier) return;
    S.renderTier=next;
    if(S.raf){ cancelAnimationFrame(S.raf); S.raf=0; }
    S.last=0;
    resize();
    if(next!=='static'&&S.active) draw(frameTime());
    if(next==='balanced') kick();
  });
  window.addEventListener('site32:appearance-change',()=>{
    S.active=presentationMode()!=='minimal';
    if(S.raf){ cancelAnimationFrame(S.raf); S.raf=0; }
    ...
    buildParticles();
    if(S.active) draw(frameTime());
    else if(ctx) ctx.clearRect(0,0,S.w,S.h);
    kick();
  });
  if(motion.addEventListener) motion.addEventListener('change',e=>{ S.reduce=MOTION_FORCE_REDUCE||!!e.matches; buildParticles(); if(S.active) draw(frameTime()); kick(); });
  ...
  resize();
  kick();
  window.__site32BackgroundState=()=>({presentationMode:presentationMode(),canvasActive:S.active,rafActive:!!S.raf,renderTier:S.renderTier,reducedMotion:S.reduce});
}
...
async function boot0(){
  ...
  revealView(land); maybeOnboard(); try{ applyI18n(i18nBootLang()); }catch(e){}
  applyAudienceRole('public');
  fetch('/api/me',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject(new Error('identity unavailable'))).then(me=>{
    window._role=me.role;
    applyAudienceRole(me.role||'public');
    ...
    startRoleBoundRuntime();
  }).catch(()=>{ window._role=''; applyAudienceRole('public'); });
}
buildDots();
boot0();

/* ---- U: 返回顶部 (每个 .page 内部独立滚动, 用捕获监听) ---- */
function totopGo(){ const p=document.querySelector('.page.show'); if(p) p.scrollTo({top:0,behavior:motionReduced()?'auto':'smooth'}); }
(function(){
...
function init3D(){
  try{
    ...
    let t=0, threeRaf=0, renderTier=r4RenderTier();
    const reducedSceneMotion=!!(window.matchMedia&&window.matchMedia('(prefers-reduced-motion: reduce)').matches);
    function scheduleThree(){
      if(cur!=='home'||document.hidden) return;
      if(reducedSceneMotion||renderTier!=='balanced'){ renderer.render(scene,cam); return; }
      if(!threeRaf) threeRaf=requestAnimationFrame(loop);
    }
    window.threeSceneKick=scheduleThree;
    document.addEventListener('visibilitychange', scheduleThree);
    window.addEventListener('r4renderchange',e=>{
      renderTier=(e.detail&&e.detail.tier)||r4RenderTier();
      if(renderTier!=='balanced'&&threeRaf){ cancelAnimationFrame(threeRaf); threeRaf=0; }
      renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,renderTier==='balanced'?2:1));
      resize();
      scheduleThree();
    });
    function loop(){
      threeRaf=0;
      if(reducedSceneMotion||renderTier!=='balanced'||cur!=='home'||document.hidden){ scheduleThree(); return; }
      t+=0.016; idle+=0.016;
      ...
      if(cur==='home' && !document.hidden) renderer.render(scene,cam);   // 只在总览可见时渲染省 GPU
      if(cur==='home' && !document.hidden && !reducedSceneMotion && renderTier==='balanced') threeRaf=requestAnimationFrame(loop);
    }
    scheduleThree();
...
window.init3DRealRig=function(){
  try{
    ...
    let W=host.clientWidth||560, H=Math.round(W*0.62);
    const renderer=new THREE.WebGLRenderer({canvas:cv, antialias:true, alpha:true});
    const motionPref=window.matchMedia?window.matchMedia('(prefers-reduced-motion: reduce)'):{matches:false};
    let reducedMotion=motionReduced(), sceneVisible=true, renderTier=r4RenderTier();
    const lowPower=((navigator.hardwareConcurrency||8)<=4)||((navigator.deviceMemory||8)<=4);
    const balancedDprCap=lowPower?1.1:(window.innerWidth<=1536?1.35:1.5);
    const sceneDprCap=()=>renderTier==='balanced'?balancedDprCap:1;
    renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,sceneDprCap())); renderer.setSize(W,H);
    renderer.shadowMap.enabled=true; renderer.shadowMap.type=THREE.PCFSoftShadowMap;
    ...
    key.shadow.mapSize.set(lowPower?768:1024,lowPower?768:1024); const sc=key.shadow.camera; sc.left=-12; sc.right=12; sc.top=10; sc.bottom=-9; scene.add(key);
    ...
    window.focus3D=function(k){ const f=FOCUS[k]; if(!f) return; tRY=f.ry; tRX=.06; idle=-8; pulseG=f.g; pulseT=1.5; toast('⬢ 3D 聚焦: '+({lab:'AI 脑机架',car:'车载脑',arm:'主机械臂工位',xrd:'XRD/PL 表征站',vps:'VPS/公网层',sample:'样品路径'})[k]); if(reducedMotion||renderTier!=='balanced') scheduleThree(); };
    cv.addEventListener('pointerdown',e=>{drag=true;lx=e.clientX;ly=e.clientY;idle=0;cv.style.cursor='grabbing';});
    window.addEventListener('pointerup',()=>{drag=false;cv.style.cursor='grab';});
    window.addEventListener('pointermove',e=>{ if(!drag)return; tRY+=(e.clientX-lx)*.01; tRX=Math.max(-.45,Math.min(.5,tRX+(e.clientY-ly)*.005)); lx=e.clientX; ly=e.clientY; idle=0; if(renderTier!=='balanced') scheduleThree(); });
    function resize(){ const w=host.clientWidth||W,h=Math.round(w*.62); renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,sceneDprCap())); renderer.setSize(w,h); cam.aspect=w/h; cam.updateProjectionMatrix(); if(reducedMotion||renderTier!=='balanced') renderer.render(scene,cam); }
    window.addEventListener('resize',resize);
    ...
    let t=0, threeRaf=0, lastFrame=0;
    const frameStep=1000/(lowPower?24:36);
    function scheduleThree(){
      if(cur!=='home'||document.hidden||!sceneVisible) return;
      if(reducedMotion||renderTier!=='balanced'){ renderer.render(scene,cam); return; }
      if(!threeRaf) threeRaf=requestAnimationFrame(loop);
    }
    window.threeSceneKick=scheduleThree;
    document.addEventListener('visibilitychange', scheduleThree);
    if(window.IntersectionObserver){
      const sceneObserver=new IntersectionObserver(entries=>{
        ...
        if(sceneVisible) scheduleThree();
      },{root:null,threshold:.02});
      sceneObserver.observe(host);
    }
    if(motionPref.addEventListener) motionPref.addEventListener('change',e=>{
      reducedMotion=MOTION_FORCE_REDUCE||!!e.matches;
      if(reducedMotion&&threeRaf){ cancelAnimationFrame(threeRaf); threeRaf=0; }
      scheduleThree();
    });
    window.addEventListener('r4renderchange',e=>{
      renderTier=(e.detail&&e.detail.tier)||r4RenderTier();
    ...
    function loop(frameTs){
      threeRaf=0;
      if(reducedMotion||renderTier!=='balanced'||cur!=='home'||document.hidden||!sceneVisible){ scheduleThree(); return; }
      if(lastFrame && frameTs-lastFrame<frameStep){ threeRaf=requestAnimationFrame(loop); return; }
      const dt=Math.min(.05,Math.max(.001,lastFrame?(frameTs-lastFrame)/1000:.016));
      lastFrame=frameTs; t+=dt; idle+=dt;
      const frameScale=dt/.016;
      if(!reducedMotion){
        wheels.forEach(w=>w.rotation.x-=.09*frameScale); sweep.rotation.y+=.045*frameScale; rackCore.rotation.x+=.01*frameScale; rackCore.rotation.y+=.013*frameScale;
        ...
        statusRings.forEach((r,i)=>{ r.material.opacity=.15+.12*Math.sin(t*1.1+i*.9)*Math.sin(t*1.1+i*.9); r.rotation.z+=(.002+i*.0005)*frameScale; });
      ...
      if(cur==='home'&&!document.hidden&&!reducedMotion&&sceneVisible&&renderTier==='balanced') threeRaf=requestAnimationFrame(loop);
    }
    scheduleThree();
  }catch(e){
    console.error('[Site32 home scene] Real rig initialization failed; using legacy fallback.', e);
    init3D();
  }
};
var _threeSceneStarted=false;
var _threeHomeInitialized=false;
var _threeSceneLoadScheduled=false;
function initHomeThreeOnce(){
  // boot0() runs before the real-rig assignment below is reached. Do not start the
  // legacy renderer during that short window or both renderers will own #c3d.
  if(_threeHomeInitialized||typeof window.init3DRealRig!=='function'||!window.THREE||cur!=='home'||r4RenderTier()!=='balanced') return;
  _threeHomeInitialized=true;
  window.init3DRealRig();
}
function scheduleThreeSceneLoad(){
  if(r4RenderTier()!=='balanced') return;
  if(window.THREE){ initHomeThreeOnce(); return; }
  if(_threeSceneStarted||_threeSceneLoadScheduled) return;
  _threeSceneLoadScheduled=true;
  const start=()=>{
    _threeSceneLoadScheduled=false;
    if(cur==='home'&&!document.hidden&&r4RenderTier()==='balanced') loadThreeSceneOnce();
  };
  start();
}
function loadThreeSceneOnce(){
  ...
  s.src='/three.min.js';
  s.async=true;
  s.onload=function(){ initHomeThreeOnce(); window.dispatchEvent(new CustomEvent('three-library-ready')); };
  document.head.appendChild(s);
}
window.ensureThreeLibrary=loadThreeSceneOnce;
window.addEventListener('r4renderchange',e=>{
  if(e.detail&&e.detail.tier==='balanced') scheduleThreeSceneLoad();
});
...
function _palShellItems(){
  return [
    ...
    {ic:'RB', t:'机器人 / 执行: 车载脑与机械臂工位', k:'robot execution vehicle car arm workstation task 机器人 执行 车载脑 机械臂', f:()=>go('twin')},
...
function palOpen(){
  if(!hasReviewerAccess()){
    go('home');
    const search=document.getElementById('homeSearchInput'); if(search) search.focus();
    return;
  }
...
window.addEventListener('keydown', e=>{
  const semanticTarget=e.target&&e.target.closest
    ? e.target.closest('[role="button"][tabindex="0"],[role="tab"][tabindex="0"]') : null;
  if(semanticTarget && (e.key==='Enter'||e.key===' ')){
    e.preventDefault(); semanticTarget.click(); return;
  }
  if((e.ctrlKey||e.metaKey) && (e.key==='k'||e.key==='K')){ e.preventDefault();
    if(!hasReviewerAccess()){
      go('home'); const search=document.getElementById('homeSearchInput'); if(search) search.focus();
      return;
    }
  ...
  if(!_singleKeyShortcutsEnabled() && !e.ctrlKey && !e.metaKey && !e.altKey && e.key.length===1) return;
...
function kbdToggle(){
  ...
  if(open){ const b=document.getElementById('kbdBody');
    if(b) b.innerHTML=KBD_SHORTCUTS.map(([k,v])=>'<div class="kbd-row"><kbd>'+esc(k)+'</kbd><span>'+esc(v)+'</span></div>').join('')
      ...
      +'<label class="kbd-dens kbd-shortcut-toggle"><span>单键快捷键</span><input type="checkbox" id="kbdSingleKeys" onchange="singleKeyToggle(this.checked)"><span class="kbd-toggle-state" id="kbdSingleKeysState"></span></label>';
    _densBtnSync(); _singleKeyBtnSync(); }
}
...
function _singleKeyShortcutsEnabled(){
  try{ return localStorage.getItem('xrd_a11y_single_key_shortcuts')!=='0'; }catch(e){ return true; }
}
function singleKeyToggle(enabled){
  try{ localStorage.setItem('xrd_a11y_single_key_shortcuts',enabled?'1':'0'); }catch(e){}
  if(window.xrdR4Accessibility) window.xrdR4Accessibility.setSingleKeyShortcutsEnabled(enabled);
  _gPending=false; clearTimeout(_gTimer); _singleKeyBtnSync();
  toast('单键快捷键: '+(enabled?'已开启':'已关闭'),'info');
}
function _singleKeyBtnSync(){
  const input=document.getElementById('kbdSingleKeys'), state=document.getElementById('kbdSingleKeysState');
  const enabled=_singleKeyShortcutsEnabled();
