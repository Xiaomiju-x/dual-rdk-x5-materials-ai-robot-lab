...
var rafId=null, lastFrame=0, forceFrame=true;

function renderTier(){
  var t=document.body && document.body.dataset.r4Render;
  if(!t && window.R4Performance && window.R4Performance.getTier) t=window.R4Performance.getTier();
  return /^(balanced|lite|static)$/.test(t||'') ? t : 'balanced';
}
function frameMs(){ return renderTier()==='balanced' ? 1000/30 : 1000/24; }
function damp(rate, frames){ return 1-Math.pow(1-rate, Math.max(0,frames)); }
function scheduleFrame(force){
  if(force) forceFrame=true;
  if(!visible || !ok3d || document.hidden || rafId!==null) return;
...
function applyRenderTier(){
  if(!renderer) return;
  var tier=renderTier(), cap=tier==='balanced'?1.5:(tier==='lite'?1.15:1);
  renderer.setPixelRatio(Math.min(window.devicePixelRatio||1,cap));
  renderer.shadowMap.enabled=tier==='balanced';
  forceFrame=true;
}
...
function init(){
  ...
  try{
    ...
    window.addEventListener('pointermove',function(e){ if(!drag)return;
      ...
      lx=e.clientX;ly=e.clientY;idleT=0; scheduleFrame(true); });
    cv.addEventListener('wheel',function(e){ e.preventDefault();
      camT.r=Math.max(1.2,Math.min(10,camT.r*(e.deltaY>0?1.08:0.92))); idleT=0; scheduleFrame(true); },{passive:false});
    window.addEventListener('resize',function(){
      var w=host.clientWidth||W,h=Math.max(380,Math.round(w*0.52));
      applyRenderTier(); renderer.setSize(w,h); cam.aspect=w/h; cam.updateProjectionMatrix(); scheduleFrame(true); });
...
function poll(){
  ...
  fetch(url, {cache:'no-store'}).then(function(r){return r.json();}).then(function(d){
    ...
    latest=d; renderChips(d); renderTable(d); renderMap(d); renderPlan(d); renderReplay(d); scheduleFrame(true);
  }).catch(function(){});
}
...
function loop(now){
  rafId=null;
  if(!visible || !ok3d || document.hidden) return;
  var tier=renderTier(), interval=frameMs();
  if(tier!=='static' && lastFrame && now-lastFrame<interval){ scheduleFrame(false); return; }
  if(tier==='static' && !forceFrame) return;
  var dt=lastFrame ? Math.min(0.1,(now-lastFrame)/1000) : interval/1000;
  var frameScale=dt*60, armEase=damp(0.06,frameScale), carEase=damp(0.05,frameScale);
  lastFrame=now; forceFrame=false;
  tAcc+=dt; idleT+=dt;
  ...
  if(latest){
    ['arm01','arm02'].forEach(function(a){
      ...
      for(var i=0;i<6;i++) smooth[a][i]+=(j.angles[i]-smooth[a][i])*armEase;
      if(j.gripper!=null) smooth.grip[a]+=(j.gripper-smooth.grip[a])*armEase;
    });
    var c=latest.car;
    if(c&&c.pose){
      ...
      smooth.car.x+=(tx-smooth.car.x)*carEase;
      smooth.car.yaw+=(((-c.pose.yaw)-smooth.car.yaw))*carEase;
      smooth.car.z=smooth.car.z==null?tz:smooth.car.z+(tz-smooth.car.z)*carEase;
    }
  }
  ...
  if(carG){
    ...
    carParts.wheels.forEach(function(w){ w.rotation.z-=0.06*frameScale; });
    carParts.sweep.rotation.y+=0.05*frameScale;
    /* 尾迹 */
    if(tAcc%0.2<0.017){
  ...
  if(rackCore){ rackCore.rotation.y+=0.012*frameScale;
  ...
  if(sampleG){
    ...
    var sampleEase=damp(0.08,frameScale);
    sampleG.position.x+=(sampleTarget.x-sampleG.position.x)*sampleEase;
    sampleG.position.z+=(sampleTarget.z-sampleG.position.z)*sampleEase;
    sampleG.position.y=0.13+Math.sin(tAcc*3)*0.018;
    sampleG.rotation.y+=0.035*frameScale;
  }
  if(pathLine && pathLine.material) pathLine.material.opacity=0.38+Math.sin(tAcc*1.7)*0.12;
  if(xrdNode) xrdNode.rotation.y+=0.004*frameScale;
  if(plNode) plNode.rotation.y-=0.006*frameScale;
  ...
  if(camT.fpv && carG){
    ...
    if(tier!=='static') scheduleFrame(false);
    return;
  }
  /* 相机: 球坐标 lerp + 跟车 + 闲转 */
  if(camT.follow==='car' && carG){ camT.cx=carG.position.x; camT.cz=carG.position.z; }
  if(!drag && idleT>6) camT.th+=0.0018*frameScale;
  ['r','th','ph','cx','cy','cz'].forEach(function(k){ camC[k]+=(camT[k]-camC[k])*armEase; });
  cam.position.set(
...
document.addEventListener('visibilitychange',function(){
  ...
  else if(visible){ poll(); scheduleFrame(true); }
});
window.addEventListener('r4renderchange',function(){ applyRenderTier(); scheduleFrame(true); });

/* ---- 对外: revealView('twin') 时调 ---- */
window.twinShow=function(){
  visible=true;
  startPoll();
  if(window.THREE){ init(); scheduleFrame(true); }
  else{ /* three.min.js 还在路上 (app.js 已发起加载), 等它 */
    if(window.ensureThreeLibrary) window.ensureThreeLibrary();
    var n=0, t=setInterval(function(){
      if(!visible){ clearInterval(t); }
      else if(window.THREE){ clearInterval(t); init(); scheduleFrame(true); }
      else if(++n>50){ clearInterval(t); fail(); }
    }, 200);
  }
};
window.twinHide=function(){ visible=false; stopPoll(); cancelFrame(); };
})();
