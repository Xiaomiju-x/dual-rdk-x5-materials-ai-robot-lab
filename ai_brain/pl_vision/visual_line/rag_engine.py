...
class RAGEngine:
    ...
    v4 Round 4: 优先使用 xrd/spectrum_knowledge_shared/embeddings/ (2462 篇 NIR 荧光粉论文,
    Round 4 产物). 若未建, fallback 到 spectrum_numerical/xrd_knowledge/embeddings/
    (Round 3 复制自 xrd_numerical 的 197 篇 XRD 论文).
    """

    # Round 4: 共享知识库路径, 兼容 PC 和 X5 两种部署布局
    # PC:  xrd/spectrum_vision/visual_line/ → 退两级到 xrd/spectrum_knowledge_shared/
    # X5:  ~/spec_vision/ → 退一级到 ~/spectrum_knowledge_shared/
    _SHARED_CANDIDATES = [
        os.path.abspath(os.path.join(_SCRIPT_DIR, "..", "..", "spectrum_knowledge_shared", "embeddings")),
        os.path.abspath(os.path.join(_SCRIPT_DIR, "..", "spectrum_knowledge_shared", "embeddings")),
    ]
    _SHARED_DIR = next(
        (d for d in _SHARED_CANDIDATES if os.path.isdir(d)),
        _SHARED_CANDIDATES[0]
    )

    def __init__(self, chunks_path=None, vectors_path=None):
        if chunks_path is None:
            # 优先共享 (Round 4) → fallback 本地 xrd_knowledge (Round 3)
            shared_chunks = os.path.join(self._SHARED_DIR, "chunks.json")
            if os.path.isfile(shared_chunks):
                chunks_path = shared_chunks
                print(f"[RAG] 使用共享 spectrum_knowledge_shared (2462 篇)", flush=True)
            else:
                chunks_path = os.path.join(_SCRIPT_DIR, "xrd_knowledge", "embeddings", "chunks.json")
                print(f"[RAG] 使用本地 xrd_knowledge (fallback)", flush=True)
        if vectors_path is None:
            if chunks_path.startswith(self._SHARED_DIR):
                vectors_path = os.path.join(self._SHARED_DIR, "vectors.npy")
            else:
        ...
        # TF-IDF 延迟构建: 2462 chunks × ~30k vocab 稠密矩阵 ~3GB, 断网 fallback 才真用
        # 正常联网走 DashScope 向量路径, TF-IDF 矩阵永远不建 → X5 省 ~2.5GB RSS
