...
int main(int argc, char **argv) {
  ...
  // USB hub 掉电重枚举后串口 fd 永久失效, SDK 只会无限报 DATA_TIME_OUT.
  // 连续超时 ~5s (50 次 @10Hz) 就退出进程, 由 launch 层 respawn 重启,
  // 重启后重新 open /dev/LD14 (udev 已重建符号链接) 即恢复. (2026-06-10)
    ...
    switch (ldlidarnode->GetLaserScanData(laser_scan_points, 1500)){
      ...
      case ldlidar::LidarStatus::DATA_TIME_OUT:
        ...
        if (++timeout_streak >= 50) {
          RCLCPP_ERROR(node->get_logger(), "consecutive timeout ~5s, exiting for respawn (stale fd after USB re-enumeration).");
          exit(EXIT_FAILURE);
        }
        break;
